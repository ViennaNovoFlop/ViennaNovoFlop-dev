...
#define MEMBERNAME "IsType"
  ...
  buf[0]='\0'; // Safety
  fgets(buf,sizeof(buf),test_fptr);
...
#define MEMBERNAME "ReadLine"
  ...
  do {
    ...
    if(readbuf[SCRATCH_BUF_SIZE-2]!='\0') {
      ...
      for(int i=0;i<SCRATCH_BUF_SIZE;i++) {
        if(readbuf[i]>0x7E) {
          buf=""; // Clear buf
          return 1;
        }
...
#define MEMBERNAME "ReadVec3f"
  ...
  if(datastyle == vf_obin4)      {  // 4-byte binary type
    ...
    if(count<Nin) {
      ...
      if(fseek(infile,(count-Nin)*sizeof(OC_REAL4),SEEK_CUR)!=0) {
    ...
    if(count<Nin) {
      // Skip extra values in input file.
      if(fseek(infile,(count-Nin)*sizeof(OC_REAL8),SEEK_CUR)!=0) {
        return 1; // Presumably EOF
      }
    }
...
{ // Returns -1 if mesh type not supported,
  ...
  fh.Nx=xdim; fh.Nz=ydim; fh.Ny=zdim;  // Note swap of y & z
  fh.dimen=3;
...
#define MEMBERNAME "IsType"
  ...
  char buf[8];
  fread(buf,1,8,test_fptr);
  if(memcmp(VecFileMagic02,buf,8)==0) return 1;  // Match!
  return 0;
#undef MEMBERNAME
}
...
{ // Fills header fields from file.  Throws an exception
  ...
  do {
    ...
    if(Tcl_Gets(inchan,&linebuf)<0) {
      ...
      OC_THROW(Oc_Exception(__FILE__,__LINE__,
                            "Vf_Ovf20FileHeader","ReadHeader",
                            256,"Premature EOF reading header."));
    }
    cptr = Tcl_DStringValue(&linebuf);
    CollapseHeaderLine(cptr);
  } while(cptr[0]=='\0' ||
...
(Tcl_Channel outchan,Vf_OvfFileVersion version) const
{
  if(!IsValid()) {
    OC_THROW(Oc_Exception(__FILE__,__LINE__,
                          "Vf_Ovf20FileHeader","WriteHeader",
  ...
  if(version == vf_ovf_invalid) {
    OC_THROW(Oc_Exception(__FILE__,__LINE__,
                          "Vf_Ovf20FileHeader","WriteHeader",
                          256,"Invalid file version request"));
  }

  if(version != vf_ovf_10 && version != vf_ovf_20) {
    OC_THROW(Oc_Exception(__FILE__,__LINE__,
                          "Vf_Ovf20FileHeader","WriteHeader",
                          256,"Unsupported file version request"));
  }

  if(version == vf_ovf_10 && valuedim.Value() != 3) {
    OC_THROW(Oc_Exception(__FILE__,__LINE__,
             "Vf_Ovf20FileHeader","WriteHeader",256,
             "OVF 1.0 format only supports output of 3-vectors"));
  }

  if(version == vf_ovf_10) {
    if(meshtype.Value() == vf_ovf20mesh_rectangular) {
  ...
  for(vector<Vf_Ovf20FieldWrap*>::const_iterator it = field_list.begin();
    ...
    if(version == vf_ovf_10) {
      ...
      if((*it)->GetNameString().compare("valueunits")==0) {
        ...
        int namelen = strlen("valueunit");
        int valuelen = vu[0].size();
    ...
    int namelen = strlen((*it)->GetName());
    int valuelen = valuebuf.size();
...
(Tcl_Channel outchan,
 ...
 OC_BOOL bare,
 Vf_OvfFileVersion version) const
{
  // Dump data
  ...
  if(datastyle == vf_oascii) {
    ...
    if(!bare) {
      ...
      if(version != vf_ovf_10) Oc_Flip4(&check);
#else                    // Input is LSB
      if(version == vf_ovf_10) Oc_Flip4(&check);
#endif
      if(Nb_WriteChannel(outchan,reinterpret_cast<const char*>(&check),
        ...
        OC_THROW(Oc_Exception(__FILE__,__LINE__,
                              "Vf_Ovf20FileHeader","WriteData",
                              256,"Write error (device full?)"));
      }
    }

    const int stride = 1024; // Number of nodes blocked together for output
    int record_count = stride;
...
#if (OC_BYTEORDER != 4321)  // Input is MSB
      if(version != vf_ovf_10) Oc_SwapWords4(d4buf,outwords);
#else                    // Input is LSB
      if(version == vf_ovf_10) Oc_SwapWords4(d4buf,outwords);
#endif
    ...
    if(!bare) {
      ...
      Vf_OvfFileFormatSpecs::GetCheckValue(check);
#if (OC_BYTEORDER != 4321)  // Input is MSB
      if(version != vf_ovf_10) Oc_Flip8(&check);
#else                    // Input is LSB
      if(version == vf_ovf_10) Oc_Flip8(&check);
#endif
...
#if (OC_BYTEORDER != 4321)  // Input is MSB
        if(version != vf_ovf_10) Oc_SwapWords8(dout,outwords);
#else                    // Input is LSB
        if(version == vf_ovf_10) Oc_SwapWords8(dout,outwords);
#endif
