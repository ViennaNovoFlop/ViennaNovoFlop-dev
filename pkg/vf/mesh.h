...
public:
  ...
  virtual OC_REAL4m GetApproximateCellWidth() const =0;
  ...
  virtual OC_REAL4m GetPreferredStepSize(OC_REAL4m step) const =0;
  /// Import "step" and return value is in mesh units.  For regular grids
  /// the return value should probably be an integral multiple of
  /// "GetApproximateCellWidth()".

  virtual OC_REAL4m GetSubsampleGrit(void) const =0;
  /// Used to set increment on subsample scale widget in display control
  /// window.  Units are RELATIVE to "GetApproximateCellWidth()".  For
  /// regular grids it doesn't make sense to sample at a rate smaller
  /// than the grid lattice spacing, so this value should be 1 (provided
  /// GetApproximateCellWidth() returns the lattice spacing constant),
  /// but for irregular meshes, a smaller value may make perfect sense.
...
public:
  ...
  OC_REAL4m GetApproximateCellWidth() const { return OC_REAL4m(1.); }
  Nb_Vec3<OC_REAL4>  GetApproximateCellDimensions() const {
    return Nb_Vec3<OC_REAL4>(OC_REAL4(1.),OC_REAL4(1.),OC_REAL4(1.));
  }
  OC_REAL4m GetPreferredStepSize(OC_REAL4m step) const {
    return OC_REAL4m(OC_ROUND(step));
  }
...
private:
  ...
  Nb_Vec3<OC_REAL8> NodalToProblemCoords(Nb_Vec3<OC_REAL8> const &w_nodal) const;
  Nb_Vec3<OC_REAL8> ProblemToNodalCoords(Nb_Vec3<OC_REAL8> const &v_problem) const;
...
public:
  ...
  Vf_GridVec3f(const char *filename,const char *title,const char *desc,
	       ...
	       const char *valueunit,OC_REAL8 valuemultiplier,
               ...
               const Nb_Vec3<OC_REAL8> &basept,const Nb_Vec3<OC_REAL8> &gridstep,
  ...
  inline const Nb_Vec3<OC_REAL8>& GridVec(OC_INDEX i,OC_INDEX j,OC_INDEX k) const {
    return grid(k,j,i);
  }
  inline const Nb_Vec3<OC_REAL8>& operator()(OC_INDEX i,OC_INDEX j,OC_INDEX k) const {
    return GridVec(i,j,k);
  }
  Nb_Vec3<OC_REAL8> Position(OC_INDEX i,OC_INDEX j,OC_INDEX k) const {
    return NodalToProblemCoords(Nb_Vec3<OC_REAL8>(OC_REAL8(i),OC_REAL8(j),OC_REAL8(k)));
  }
  ...
  virtual OC_REAL4m GetApproximateCellWidth() const {
    return static_cast<OC_REAL4m>(GetStdStep());
  }
  ...
  OC_REAL4m GetPreferredStepSize(OC_REAL4m step) const {
    OC_REAL4m basestep=static_cast<OC_REAL4m>(GetStdStep());
    if(basestep<=0.) basestep=1.0; // Safety
    return basestep*OC_REAL4m(OC_ROUND(step/basestep));
  }
...
public:
  ...
  Nb_Vec3<OC_REAL4>  GetApproximateCellDimensions() const;
  OC_REAL4m GetPreferredStepSize(OC_REAL4m step) const {
    OC_REAL4m cellsize=GetApproximateCellWidth();
    return cellsize*OC_REAL4m((OC_ROUND(step/cellsize)));
  }
  OC_REAL4m GetSubsampleGrit(void) const { return OC_REAL4m(0.1); }
  /// This is suppose to be a sensible lower bound on the sampling
  /// period relative to "GetApproximateCellWidth()".  This should
  /// probably be set to the smallest neighbor distance in the
  /// mesh, but for now we'll just use 0.1.
  virtual OC_INT4m ColorQuantityTypes(Nb_List<Nb_DString> &types) const;
  virtual OC_INT4m ColorQuantityTransform
  (const Nb_DString flipstr,
   const Nb_DString& quantity_in,OC_REAL8m phase_in,OC_BOOL invert_in,
