...
//   Currently this routine maps things through %x or %lx, but this
// may change in the future to use a hash table.  The client should
// first call GetAsciiSize to determine how large a buffer is needed
// to hold the ascii-fied pointer string, then call PtrToAscii to
// get a string representation of the ptr.  This can be passed across
// to Tcl procs as needed.  Later, when a C routine is called by a
// Tcl proc having such a string, this string can be converted back
// to a valid ptr via AsciiToPtr.  NOTE: AsciiToPtr should ONLY be
// called with strings produced by PtrToAscii.  There is currently
// no check on this, but there will be if this class gets modified
// to use a hash table lookup scheme.
//
class Omf_AsciiPtr
{
private:
  ...
  union ptr_int_hack { void* ptr; unsigned long lval; };
public:
  ...
  /// that buf is dimensioned to at least GetStringSize() bytes.
...
public:
  ...
  operator double() const; // Returns time in seconds, in floating point
