 ...
 *
 * Last modified on: $Date: 2008-10-08 03:56:36 $
...
NbTclObjPtr& Nb_TclObjArray::operator[](int index)
{
#ifndef NDEBUG
  if(index>=size || index<0) {
    OC_THROW(Oc_Exception(__FILE__,__LINE__,"Nb_TclObjArray",
                          "operator[]",NB_TCLOBJARRAY_ERRBUFSIZE,
...
const NbTclObjPtr& Nb_TclObjArray::operator[](int index) const
{
#ifndef NDEBUG
  if(index>=size || index<0) {
    OC_THROW(Oc_Exception(__FILE__,__LINE__,"Nb_TclObjArray",
                          "operator[] const",NB_TCLOBJARRAY_ERRBUFSIZE,
                          "Array out-of-bounds; index=%d, size=%d",
                          index,size));
  }
#endif
