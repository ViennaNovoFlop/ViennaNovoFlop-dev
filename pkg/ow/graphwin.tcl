...
#
# Last modified on: $Date: 2008-02-05 22:42:33 $
...
Oc_Class Ow_GraphWin {
    ...
    callback method ZoomBoxDo { x2 y2 } { graph \
              zoombox_yaxis zoombox_basept zoombox_minsize \
              xgraphmin xgraphmax \
	    ...
	    set mindiff [expr {$sum*pow(10,-1*($tcl_precision-3))}]
	    if {$diff<$mindiff} {
	...
	if {$tcl_precision<15} {
	    ...
	    # If tcl_precision is 17, then effective
