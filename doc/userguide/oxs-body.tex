...
\hyperrefhtml{\MIF~2.1 format}{\MIF~2.1 format (Sec.~}{)}{sec:mif2format}.
...
(Sec.~}{)}{sec:mifconvert} for conversion to \MIF~2.1 format
``on-the-fly.''
...
\htmlref{\MIF~2.1 file}{sec:mif2format},
...
(Sec.~}{)}{sec:mifconvert} for implicit conversion to \MIF~2.1 format.
...
\item[\optkey{-parameters params}]
  Sets \hyperrefhtml{\MIF~2.1}{\MIF~2.1 (Sec.~}{)}{sec:mif2format} file
...
\item[\optkey{-restart \boa 0\pipe 1\bca}]
...
in the \hyperrefhtml{\MIF~2.1}{\MIF~2.1 (Sec.~}{)}{sec:mif2format}
format, or convertible to that format by the command line tool
\hyperrefhtml{\app{mifconvert}}{\app{mifconvert}
(Sec.~}{)}{sec:mifconvert}.  There are sample \MIF~2.1 files in the
...
\htmlref{\MIF~2.1 file}{sec:mif2format} specified on the command line
...
the \hyperrefhtml{\MIF~2.1}{\MIF~2.1 (Sec.~}{)}{sec:mif2format} format,
...
\hyperrefhtml{\MIF~2.1 file.}{\MIF~2.1 file (Sec.~}{).}{sec:mif2format}
...
\MIF~2.1 file format documentation.  The value \oxsval{args\_request}
...
\item[Oxs\_UniaxialAnisotropy:]
   ...
   parameters, crystalline anisotropy constant \oxslabel{K1} (in
   \latexhtml{J/m${}^3$}{J/m^3}) and anisotropy direction \oxslabel{axis}.
   The axis direction is an easy axis if K1$>$0, or is the normal to the
   easy plane if K1$<0$.  Both may be varied cellwise across the mesh.
   The anisotropy constant is initialized with a scalar field object,
   and the axis direction with a vector field object.  The axis
   direction must be non-zero at each point, and will be normalized to
   unit magnitude before being used.  The energy computed by this term
   is non-negative in all cases.
...
\item[Oxs\_CubicAnisotropy:]
   ...
   parameters, crystalline anisotropy constant \oxslabel{K1} (in
   \latexhtml{J/m${}^3$}{J/m^3}) and anisotropy directions
   \oxslabel{axis1} and \oxslabel{axis2}.  The third anisotropy axis is
   computed as the vector product, $\cd{axis1}\times\cd{axis2}$.  The
   axis directions are easy axes if K1$>$0, or hard axes if K1$<$0.  All
   may be varied cellwise across the mesh.  K1 is initialized with a
   scalar field object, and the axis directions with vector field
   objects.  The \cd{axis1} and \cd{axis2} directions must be
   mutually orthogonal and non-zero at each point.  For each cell, if
   K1$>$0 then the computed energy will be non-negative, else if K1$<$0
   then the computed energy will be non-positive.
...
\item[Oxs\_ScriptUZeeman:]
   ...
   \html{section} of the \MIF~2.1 file format documentation.  The value
...
\item[Oxs\_TransformZeeman:]
   ...
   \hyperrefhtml{User Defined Support Procedures}{User Defined Support
   Procedures section (Sec.~}{)}{par:supportProcs}\html{ section} of the
   \MIF~2.1 file format documentation.  The value for \cd{script\_args}
...
\abovemath{\gamma>0}, you should either set \abovemath{\alpha<=0} to
force spins back towards the effective field, or disable the energy
precision control (discussed \htmlonlyref{below}{html:oxsrkeprecision}).
...
Specify Oxs\_TimeDriver:\oxsval{name} \ocb\\
 ...
 \bi basename \oxsval{base\_file\_name}\\
 ...
 \bi scalar\_output\_format \oxsval{format}\\
 \bi vector\_field\_output\_format \ocb\oxsval{ style precision }\ccb\\
...
The value associated with \oxslabel{basename} is used as a prefix for
...
\oxslabel{scalar\_output\_format} should be a C-style printf string specifying
the output format for DataTable output .  This is
optional, with default value ``\cd{\%.17g}''.  The value associated with
\oxslabel{vector\_field\_output\_format} should be a two element list,
specifying the style and precision for vector field output sent to
\hyperrefhtml{\app{mmDisp}}{\app{mmDisp}
(Sec.~}{)}{sec:mmdisp}\index{application!mmDisp} and
\hyperrefhtml{\app{mmArchive}}{\app{mmArchive}
(Sec.~}{)}{sec:mmarchive}\index{application!mmArchive}.  The first
...
\cd{scalar\_output\_format}.  The default value for
...
2.1}{sec:mif2format} documentation,}
\latex{the MIF 2.1 documentation (Sec.~\ref{par:oxsExtReferencing},
...
\end{rawhtml}
   ...
   (Sec.~}{)}{par:supportProcs} \html{section} of the \MIF~2.1 file
