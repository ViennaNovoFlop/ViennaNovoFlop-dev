...
\item[\optkey{-restart \boa 0\pipe 1\bca}]
  \index{simulation~2D!restarting} Affects the behavior of the solver
...
\item[\optkey{-restart \boa 0\pipe 1\bca}]
  ...
  restart\index{simulation~2D!restarting} a previous run from the last
...
\verb+rsh+\index{application!rsh} command or
equivalent\index{requirement!rsh}.
...
To launch \app{batchslave} on a remote host, use \fn{rsh}\index{application!rsh}
...
this to work you must have \cd{rsh} configured properly.  In the future
...
like \cd{rsh}\index{application!rsh}.)
