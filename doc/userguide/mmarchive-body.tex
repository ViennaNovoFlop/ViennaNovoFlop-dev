\section{Data Archive: mmArchive}\label{sec:mmarchive}%
...
\app{mmArchive} does not present a user interface window of its
own, but like {\hyperrefhtml{\app{mmSolve2D}}{\app{mmSolve2D}
(Sec.~}{)}{sec:mmsolve2d}}\index{application!mmSolve2D} relies on
{\hyperrefhtml{\app{mmLaunch}}{\app{mmLaunch}
(Sec.~}{)}{sec:mmlaunch}}\index{mmLaunch~user~interface} to provide an
interface on its behalf.  Because \app{mmArchive} does not require a
window, it is possible on \Unix\ systems to bring down the X (window)
server\index{platform!Unix!X~server}
and still keep \app{mmArchive} running in the background.

\ssechead{Launching}
\app{mmArchive} may be started either by selecting the
\btn{mmArchive} button on \htmlref{\app{mmLaunch}}{sec:mmlaunch}, 
...
\ssechead{Outputs}\index{data!save}
...
\ssechead{Controls}
The \btn{Exit} in the \app{mmLaunch}-supplied user interface terminates
\app{mmArchive}.  Simply closing the user interface window does not
terminate \app{mmArchive}, but only hides the control window.  To kill
\app{mmArchive} the \btn{Exit} button must be pressed.
