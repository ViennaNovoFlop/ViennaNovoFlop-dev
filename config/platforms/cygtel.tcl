...
if {[string match g++* $ccbasename]} {
   ...
   # (or missing) on some machines.  If the following option is set to
   # 1, then then the standard double versions of floor() and ceil()
   # will be used instead.  The double versions don't provide the same
   # range as the long double versions, but the accuracy is not
   # affected if the operand is inside the double range.  This is not
   # an issue unless you have selected "long double" in one of the
   # preceding two stanzas.  If you have, though, then you should
   # check this sample test case: see what floorl(-0.253L) and
   # x-floorl(x) with x=-0.253L return.  If you get 0 for the first or
   # NAN for the second, then you should enable this option.  (Note:
   # Pimake will try to set this automatically based on output from
   # varinfo.  Setting this will override the automatic check.  See
   # also the program_compiler_c++_property_floorl_flag option below.)
   # $config SetValue program_compiler_c++_property_bad_wide2int 1

   # If your system is missing floorl, then setup this compiler flag
   # appropriately.  (This flag is used in building varinfo.cc)
