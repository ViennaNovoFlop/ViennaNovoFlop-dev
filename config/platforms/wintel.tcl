...
if {[string match cl $ccbasename]} {
   ...
   # gcc on MinGW seems to have some problems with SSE instructions,
   # at least when built against the ActiveTcl libraries.  This
   # probably arises from data alignment problems.  gcc 4.2 has an
   # option, -mstackrealign, that probably fixes this, but for the
   # present we adopt the easier workaround of just not using SSE.
   # Another option appears to be to use a Tcl/Tk built using gcc, as
   # opposed to the ActiveTcl builds that reportedly use Microsoft
   # Visual C++ 6 (from 1998).  -mjd, 16-Dec-2007
   ...
   $config SetValue program_compiler_c++_option_warn {format "-Wall \
        ...
        -Wno-non-template-friend -Wno-unused-function"}
