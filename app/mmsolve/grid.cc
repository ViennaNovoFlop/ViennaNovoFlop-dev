  ...
  if(DoBdryAnis) {
    for(BdryElt *bep=bdry.GetFirst();bep!=NULL;bep=bdry.GetNext()) {
      ...
      if(acoef!=0.0) {
        int bi=bep->i;
        int bk=bep->k;
        OC_REALWIDE bdot = bep->normal_dir * m[bi][bk];
...
{ // Returns  - \sum (thickness.(torque*h))/thickness_sum
  ...
  for(i=0;i<Nx;i++) for(k=0;k<Nz;k++) {
    double thick=m[i][k].GetThickness();
    if(thick>0.0) {
      temp=OC_REALWIDE(torque[i][k][0])*OC_REALWIDE(h[i][k][0])
	+ OC_REALWIDE(torque[i][k][1])*OC_REALWIDE(h[i][k][1])
	+ OC_REALWIDE(torque[i][k][2])*OC_REALWIDE(h[i][k][2]);
      temp *= thick;
      bucket.Accum(temp);
    }
  }
