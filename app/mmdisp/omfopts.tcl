...
Oc_Class OmfOpts {
    ...
    private variable btn_regular_grid = Oc_Nop;
    public variable gridtype = regular
    public variable allowed_gridcode = 1 { 
	$this GridsAllowedUpdate $allowed_gridcode
    } ;# 0 => Irreg grids only, 1 => Irreg & regular grids
	...
	set btn3 [radiobutton $winpath.btn3 -text "Binary-8" \
		...
		-variable [$this GlobalName datatype]]
	set btn4 [radiobutton $winpath.btn4 -text "Regular grid" \
		-relief flat -anchor w -value regular \
    ...
    method GridsAllowedUpdate { gridcode } \
	    { allowed_gridcode btn_regular_grid gridtype } {
	set allowed_gridcode $gridcode
	if { $allowed_gridcode != 0 } {
	    if {[string compare "disabled" \
		    [$btn_regular_grid cget -state]]==0} {
		$btn_regular_grid configure -state normal
		set gridtype "regular"
	    }
	} else {
	    $btn_regular_grid configure -state disabled
