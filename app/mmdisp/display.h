...
private:
  ...
  ///    mesh->GetApproximateCellWidth()/mesh_per_pixel;
  ...
  OC_INT4m DrawArrow(const char *canvas_name,
		  const Vf_DisplayVector &locvec,
                  OC_REAL4m arrowsize,OC_BOOL hide) const;
  OC_INT4m DrawPixel(const char *canvas_name,
		  const Vf_DisplayVector &locvec,
                  const Nb_Vec3<OC_REAL4> &pixel_dimens,
		  const char* stipple,OC_BOOL hide) const;
...
public:
  ...
  /// is given by mesh->GetApproximateCellWidth()/mesh_per_pixel;
