...
// The TclObj interfaces are available in Tcl later than 8.0; earlier
// than that we must use the string interface.  However, one can set
// the macro USE_TCLOBJS to 0 to force the use of the string interface.
// This may be useful for debugging purposes.
#ifndef USE_TCLOBJS
# if TCL_MAJOR_VERSION < 8 || (TCL_MAJOR_VERSION==8 && TCL_MINOR_VERSION==0)
#  define USE_TCLOBJS 1
# else
#  define USE_TCLOBJS 0
# endif
#endif
...
int CopyMesh(ClientData,Tcl_Interp *interp,int argc,CONST84 char** argv)
{
  ...
  // Create new mesh; if the original mesh is regular (i.e.,
...
static int IsRegularMesh(const Vf_Mesh* mesh)
{
  const char *name=mesh->GetMeshType();
  int regular=0;
  if(strcmp(name,"Vf_GridVec3f")==0 ||
     strcmp(name,"Vf_EmptyMesh")==0) {
    // Known regular meshes
    regular=1;
  } // Otherwise assume irregular
  return regular;
}

int IsRegularMesh(ClientData,Tcl_Interp *interp,int argc,CONST84 char**)
{
  Tcl_ResetResult(interp);
  if(argc!=1) {
    Oc_Snprintf(buf,sizeof(buf),
            "IsRegularMesh must be called with no arguments"
            " (%d arguments passed)",argc-1);
    Tcl_AppendResult(interp,buf,(char *)NULL);
    return TCL_ERROR;
  }
  int regular = IsRegularMesh(myMeshArray[activeMeshId]);
  Oc_Snprintf(buf,sizeof(buf),"%d",regular);
  Tcl_AppendResult(interp,buf,(char *)NULL);
  return TCL_OK;
}
...
{ // Import: (x,y,z) in display coordinates
  ...
  if(argc!=4) {
    Oc_Snprintf(buf,sizeof(buf),
                "GetMeshCellSize must be called with"
...
{ // Returns 6-tuple: xmin ymin zmin xmax ymax zmax, in mesh units.
  ...
  if(argc!=1) {
    Oc_Snprintf(buf,sizeof(buf),
            "GetMeshRange must be called with"
                            ...
                            int,CONST84 char **)
{
  ...
  int cqtypec=myFrame.GetColorQuantityTypes(cqtypes);
  if (cqtypec>0) {
                     ...
                     int argc,CONST84 char**)
{
  ...
  OC_INT4m mapcount = maps.GetSize();
  if(mapcount>0) {
    ...
    char* maplist=Tcl_Merge(mapcount,mapv);
              ...
              int argc,CONST84 char** argv)
{
  ...
  if(argc<4 || argc>6) {
    Oc_Snprintf(buf,sizeof(buf),"WriteMesh must be called with"
            ...
            " <regular|irregular> [title] [description]"
            " (%d arguments passed)",argc-1);
    Tcl_AppendResult(interp,buf,(char *)NULL);
    return TCL_ERROR;
  }
...
char* CenterStringOutput(char* bufptr,size_t bufsize,
                         int field_width,const char* str)
{
  const int strsize = static_cast<int>(strlen(str));
  if(strsize>field_width) {
    bufptr += Oc_Snprintf(bufptr,bufsize,"%s",str);
  } else {
    const int lmargin = (field_width - strsize)/2;
    const int rmargin = field_width - lmargin - strsize;
...
WriteMeshAverages(ClientData, Tcl_Interp *interp,
                  int argc, CONST84 char** argv)
{
  Nb_DString colhdrfmt,linefmt;
// #define DEFAULT_NUM_FMT "%- #24.17g"
# define DEFAULT_NUM_FMT "%- #20.15g"
  Tcl_Channel channel;
  int mode;
  const char* cptr;
  // Import config array:
  ...
  //   numfmt   -- Format for numeric output default is DEFAULT_NUM_FMT
  ...
  // Downcast
  Vf_GridVec3f& grid = *((Vf_GridVec3f *)myMeshArray[activeMeshId]);
    ...
    for(k=kmin;k<kmax;k++) {
      ...
      for(j=jmin;j<jmax;j++) {
        ...
        for(i=imin;i<imax;i++) {
...
#if USE_TCLOBJS
#else // Use string interface
          Oc_Snprintf(buf,sizeof(buf),
                      "set x %#.17g;  set y %#.17g;  set z %#.17g; "
                      "set r %#.17g; "
                      "set vx %#.17g; set vy %#.17g; set vz %#.17g; "
                      "set vmag %#.17g; "
                      "expr {%s}",
                      static_cast<double>(pos.x),
                      static_cast<double>(pos.y),
                      static_cast<double>(pos.z),
                      static_cast<double>(pos.Mag()),
                      static_cast<double>(scale*v.x),
                      static_cast<double>(scale*v.y),
                      static_cast<double>(scale*v.z),
                      static_cast<double>(scale*v.Mag()),
                      user_funcs[ielt].GetStr());
          // The '#' option in the format specifier is included to insure
          // that a decimal point is output.  Otherwise Tcl may interpret
          // some of these values as integers, which leads to oddities
          ...
          OC_BOOL bad_result;
          uservals[ielt][offset]
            = Nb_Atof(Tcl_GetStringResult(interp),bad_result);
          if(bad_result) {
            Tcl_AppendResult(interp,"\n---\nBad expr result: ",
  ...
  if(avetype==space) {
    ...
    if(defaultvals) {
      ...
      if(normalize && magsum>0.0) {
        m.Set(mx.GetValue()/magsum,
              my.GetValue()/magsum,
              mz.GetValue()/magsum);
      } else {
        m.Set(mx.GetValue()*scale/double(volume_point_count),
              my.GetValue()*scale/double(volume_point_count),
              mz.GetValue()*scale/double(volume_point_count));
      }
    ...
    case plane:
      switch(axis) {
        case 'x':
          ...
          if(defaultvals) tripstep = &TripleIndex::StepYZX;
        ...
        case 'y':
          ...
          if(defaultvals) tripstep = &TripleIndex::StepXZY;
        ...
        case 'z':
          ...
          if(defaultvals) tripstep = &TripleIndex::StepXYZ;
      ...
      if(defaultvals) stepcontrol = (triple_index.*tripstep)();
      else            stepcontrol = 0;
      linear_index.Step();
      while(stepcontrol<3) {
        int user_col_offset = 0;
        ...
        if(defaultvals) {
          ...
          if(normalize && magsum>0.0) {
            ...
            m.Set(mx.GetValue()*scale/double(manifold_size),
                  my.GetValue()*scale/double(manifold_size),
                  mz.GetValue()*scale/double(manifold_size));
          }
          value_column_results[OC_INDEX(0)] = m.x;
          value_column_results[OC_INDEX(1)] = m.y;
          value_column_results[OC_INDEX(2)] = m.z;
    ...
    case line:
      switch(axis) {
        ...
        case 'y':
          ...
          if(defaultvals) tripstep = &TripleIndex::StepYXZ;
        ...
        case 'z':
          ...
          if(defaultvals) tripstep = &TripleIndex::StepZXY;
              ...
              for(OC_INDEX k2=k-krad;k2<k+krad+1;k2++) {
                ...
                for(OC_INDEX j2=j-jrad;j2<j+jrad+1;j2++) {
                  ...
                  for(OC_INDEX i2=i-irad;i2<i+irad+1;i2++) {
                    ...
                    m+=grid(i2,j2,k2);
                    if(normalize) magsum += grid(i2,j2,k2).Mag();
                  }
                }
              }
              if(normalize && magsum>0.0) m *= 1.0/magsum;
              else                        m *= scale/double(ball_point_count);
              value_column_results[OC_INDEX(0)] = m.x;
              value_column_results[OC_INDEX(1)] = m.y;
              value_column_results[OC_INDEX(2)] = m.z;
              user_col_offset = 3;
          ...
          int argc, CONST84 char** argv)
{
  ...
  if ((c == 'C') && (strcmp(argv[1], "Create") == 0)) {
    ...
    if(IsRegularMesh(myMeshArray[activeMeshId])) {
    ...
    // Calculate bounding boxes, in mesh coords.  Use Zslice
    // interface to better mimic mmDisp.  (Do we always want
    // to mimic mmDisp?  Probably doesn't matter for regular
    // grids, but may for irregular grids.)
                ...
                int argc,CONST84 char** argv)
{
  ...
  // Arrow boundary (edge) parameters
  OC_BOOL ab_error;
  OC_REAL8m arrow_boundary_width = 0.0;  // Default is no boundary
  Nb_DString arrow_boundary_color = "0x000000";
  cptr = Tcl_GetVar2(interp,OC_CONST84_CHAR("plot_config"),
                     OC_CONST84_CHAR("arrow,boundarywidth"),TCL_GLOBAL_ONLY);
  OC_REAL8m ab_temp = Nb_Atof(cptr,ab_error);
  if(!ab_error) arrow_boundary_width = ab_temp;
  cptr = Tcl_GetVar2(interp,OC_CONST84_CHAR("plot_config"),
                     OC_CONST84_CHAR("arrow,boundarycolor"),TCL_GLOBAL_ONLY);
  if(!Nb_StrIsSpace(cptr)) {
    arrow_boundary_color = cptr;   // Default is 0x000000 (black)
  }
  ...
  // for regular grids, but may for irregular grids.)
  myMeshArray[activeMeshId]->GetZslice(centerpt.z - arrowspan/2,
                                       centerpt.z + arrowspan/2,
                                       temp_min.z,temp_max.z);
  Nb_BoundingBox<OC_REAL4> arrow_box(temp_min,temp_max);
  ...
  myFrame.PSDump(channel,
                 ...
                 mat_width,mat_color,
                 arrow_boundary_width,arrow_boundary_color);
 ...
 */
  ...
  Oc_RegisterCommand(interp,"IsRegularMesh",IsRegularMesh);
