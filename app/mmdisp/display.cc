...
#define MEMBERNAME "GetDisplayList"
  ...
  OC_REAL4m mesh_cellsize=mesh->GetApproximateCellWidth();
  if(mesh_cellsize<=0.0) mesh_cellsize=1.0; // Safety
...
{ // Returns a recommended subsample rate, for the current mesh at
  // current zoom.
  OC_REAL8m step=cellsize*mesh_per_pixel;
  step=mesh->GetPreferredStepSize(static_cast<OC_REAL4m>(step));
  step/=mesh->GetApproximateCellWidth();
...
OC_INT4m DisplayFrame::DrawPixel(const char *canvas_name,
			      const Vf_DisplayVector &dspvec,
			      const Nb_Vec3<OC_REAL4> &pixel_dimens,
			      const char* stipple,
			      OC_BOOL hide) const
{ // Based on code contributed by SLW
#define MEMBERNAME "DrawPixel"
  static char buf[1024];  // *Should* be plenty big enough.
  ...
  // NOTE: Timing tests on Linux 2.0.24 on a i486-DX2/66 indicate that
  //       a substantial time loss is involved inside sprintf doing %f
  //       conversions.  It is 6 or 7 times faster to use %d's instead.

  OC_REAL4m x1,x2,y1,y2;
  int r1,r2,c1,c2;

  x1=(postemp->x)-xsize/2;  x2=x1+xsize;
  r1=int(floor(x1));        r2=int(ceil(x2));
  y1=(postemp->y)-ysize/2;  y2=y1+ysize;
  c1=int(floor(y1));        c2=int(ceil(y2));
  if(hide) {
    Oc_Snprintf(buf,sizeof(buf),
		"%s create rectangle %d %d %d %d -fill %s"
		" -outline {} -tag {pixels p%d}"
		" -stipple {%s} -state hidden",
		canvas_name,r1,c1,r2,c2,color,dspvec.zslice,stipple);
  } else {
    Oc_Snprintf(buf,sizeof(buf),
		"%s create rectangle %d %d %d %d -fill %s"
		" -outline {} -tag {pixels p%d} -stipple {%s}",
		canvas_name,r1,c1,r2,c2,color,dspvec.zslice,stipple);
  }
...
OC_REAL8m DisplayFrame::GetArrowSize() const {
  OC_REAL8m arrowsize=mesh->GetApproximateCellWidth()
    * default_arrow_ratio * arrow_config.size / mesh_per_pixel;
  if(arrow_sample.subsample>1.)
    arrowsize*=arrow_sample.subsample;
  return arrowsize;
}

OC_INT4m DisplayFrame::DrawArrow(const char *canvas_name,
			      const Vf_DisplayVector &dspvec,
                              OC_REAL4m arrowsize,OC_BOOL hide) const
{
...
#define SQRT3 1.7320508075688772
  ...
  arrow_len = sqrt(delx*delx + dely*dely + delz*delz);
  if(arrow_len<OC_REAL4_EPSILON) return TCL_OK;  // This is a zero vector
  ...
  const char* hidestr="";
  if(hide) {
    hidestr="-state hidden";
  }

  const char* pcolor1=arrow_config.Color(dspvec.shade);

  // NOTE: Timing tests on Linux 2.0.24 on a i486-DX2/66 indicate that
  //       a substantial time loss is involved inside sprintf doing
  //       %f conversions.  It is 6 or 7 times faster to use %d's instead
  //       of sprintf(buf,"%s create line %6.2f %6.2f %6.2f %6.2f"
  //	             "-arrow last -fill %s",canvas_name,x1,y1,x2,y2,
  //                 colormap.TkColor[color]);
  ...
  if(proj_len>=arrow_len*0.15) { // Standard arrow
    ...
    Oc_Snprintf(buf,sizeof(buf),
            "%s create line %d %d %d %d -tag {arrows a%d}"
            " -arrow last -arrowshape {%d %d %d}"
            " -width %d -fill %s %s",
            canvas_name,r1,c1,r2,c2,dspvec.zslice,
            a1,a2,a3,linewidth,pcolor1,hidestr);
    ...
    if(vtemp->z<0) { // Arrow pointing into-plane
      ...
      int cmdlen =
	Oc_Snprintf(buf,sizeof(buf),
		    "%s create rect %d %d %d %d -tag {arrows a%d}"
		    " -fill %s -outline %s %s",
		    canvas_name,r1,c1,r2,c2,dspvec.zslice,
		    pcolor1,pcolor1,hidestr);
      if(a3>6) {
	ioff=(int)OC_ROUND(OC_REAL4m(a3)/6.);
	r1=r0-ioff;  c1=c0-ioff;
	r2=r0+ioff;  c2=c0+ioff;
	Oc_Snprintf(buf+cmdlen,sizeof(buf)-cmdlen,
		    "; %s create rect %d %d %d %d -tag {arrows a%d}"
		    " -fill %s -width 0 %s",
		    canvas_name,r1,c1,r2,c2,dspvec.zslice,
		    pcolor2,hidestr);
      ...
      int cmdlen =
	Oc_Snprintf(buf,sizeof(buf),
		    "%s create poly %d %d %d %d %d %d %d %d"
		    " -tag {arrows a%d} -fill %s -outline %s"
		    " -width %d %s",
		    canvas_name,r1,c0,r0,c2,r2,c0,r0,c1,dspvec.zslice,
		    pcolor1,pcolor1,linewidth/3,hidestr);
      // Note: The return quantity of sprintf is OS dependent.
      if(a3>6) {
	Oc_Snprintf(buf+cmdlen,sizeof(buf)-cmdlen,
		   "; %s create line %d %d %d %d -tag {arrows a%d}"
		   " -fill %s -width %d %s"
		   "; %s create line %d %d %d %d -tag {arrows a%d}"
		   " -fill %s -width %d %s",
		   canvas_name,r1,c0,r2,c0,
		   dspvec.zslice,pcolor2,linewidth/3,hidestr,
		   canvas_name,r0,c2,r0,c1,
		   dspvec.zslice,pcolor2,linewidth/3,hidestr);
      }
    }
  }
...
#define MEMBERNAME "Render"
  ...
  Tcl_SaveResult(tcl_interp, &saved);
  // Set canvas background color
  const char *fmt="%s configure -background %s";
  char *buf=new char[strlen(fmt)+strlen(canvas_name)
		    +background_color.Length()+1];
  sprintf(buf,"%s configure -background %s",canvas_name,
	  background_color.GetStr());
  if((error_code=Tcl_Eval(tcl_interp,buf)) != TCL_OK)
    goto done;
  delete[] buf;
  ...
  if(pixel_config.displaystate) {
	...
	dspvec=pixel_sample.dvlist.GetNext()) {
      if((error_code=DrawPixel(canvas_name,*dspvec,pixel_dimens,
			       pixel_config.stipple,hide))
	 != TCL_OK) {
	goto done;
      }
    }
  ...
  if(arrow_config.displaystate) {
    ...
    OC_REAL4m arrowsize=static_cast<OC_REAL4m>(GetArrowSize());
    Vf_DisplayVector *dspvec;
    for(dspvec=arrow_sample.dvlist.GetFirst();
	dspvec!=(Vf_DisplayVector *)NULL;
	dspvec=arrow_sample.dvlist.GetNext()) {
      if((error_code=DrawArrow(canvas_name,*dspvec,arrowsize,hide))
	 != TCL_OK) {
	goto done;
      }
    }
  ...
  if(draw_boundary) {
    ...
    if((error_code=DrawLine(canvas_name,boundary_list,
  ...
  Tcl_RestoreResult(tcl_interp, &saved);
  return error_code;
#undef MEMBERNAME
}
...
#define MEMBERNAME "FillBitmap"
  ...
  OC_INT4m slice_count = mesh->GetZsliceCount();
  if(slice_count<1) {
  ...
  for(OC_INT4m i=0;i<slice_count;i++) {
...
(Tcl_Channel channel,
 ...
 OC_REAL8m arrow_boundary_width,
 Nb_DString arrow_boundary_color)
{
#define MEMBERNAME "PSDump"
  ...
  OommfPSDraw psdraw(channel,
		     ...
		     arrow_boundary_width,
		     OommfPackedRGB(arrow_boundary_color.GetStr()));
...
{
  OC_REAL4m mesh_per_cell=mesh->GetApproximateCellWidth();
