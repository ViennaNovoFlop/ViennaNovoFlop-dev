...
(Tcl_Channel use_channel,
 ...
 OommfPackedRGB arrow_boundary_color
)
  : channel(use_channel),
    LineWidth(0.075),
    ArrowLength(0.90),ArrowHeadRatio(0.5),
    ArrowBoundaryWidth(arrow_boundary_width),
    ArrowBoundaryColor(arrow_boundary_color),
    InOutTipRadius(0.15),
    ...
    plot_xmax(xmax),plot_ymax(ymax)
{
  ...
  Nb_FprintfChannel(channel,NULL,1024,"  %.6g %.6g lineto\n",
                    static_cast<double>(-ArrowLength/2.),
                    static_cast<double>(-LineWidth/2.));
  if(ArrowBoundaryWidth>0.0) {
    unsigned int red,green,blue;
    ArrowBoundaryColor.Get(red,green,blue);
    Nb_WriteChannel(channel,"  closepath\n  gsave\n  fill\n  grestore\n",-1);
    Nb_FprintfChannel(channel,NULL,1024,
                      "  %.6g setlinewidth\n",
                      static_cast<double>(ArrowBoundaryWidth*0.025));
    Nb_FprintfChannel(channel,NULL,1024,
