...
Oxs_CubicAnisotropy::Oxs_CubicAnisotropy(
  ...
  : Oxs_ChunkEnergy(name,newdtr,argstr), mesh_id(0),
    K1_is_uniform(0),axis1_is_uniform(0),axis2_is_uniform(0),
    uniform_K1_value(0.)
{
  // Process arguments
 ...
 ) const
{
  ...
  if(mesh_id !=  mesh->Id()) {
    ...
    if(threadnumber != 0) {
      ...
      try {
        if(!K1_is_uniform) K1_init->FillMeshValue(mesh,K1);
  ...
  ThreeVector unifaxis1,unifaxis2;
  if(axis1_is_uniform) unifaxis1 = uniform_axis1_value;
  if(axis2_is_uniform) unifaxis2 = uniform_axis2_value;
  ...
  for(OC_INDEX i=node_start;i<node_stop;++i) {
    ...
    // very expensive, and may be to be more accurate.  At a minimum,
    // in the above expressions one should at least insure that a3^2
    // is non-negative.

    if(!K1_is_uniform) k = K1[i];
    const OC_REAL8m field_mult = (-2/MU0)*k*Ms_inverse[i];
    if(field_mult==0.0) {
