/* FILE: scriptvectorfield.cc      -*-Mode: c++-*-
 *
...
#include "meshvalue.h"
#include "director.h"
#include "scriptvectorfield.h"
#include "energy.h"		// Needed to make MSVC++ 5 happy

OC_USE_STRING;

// Oxs_Ext registration support
OXS_EXT_REGISTER(Oxs_ScriptVectorField);

/* End includes */

// Constructor
Oxs_ScriptVectorField::Oxs_ScriptVectorField(
  ...
  : Oxs_VectorField(name,newdtr,argstr)
{
  ...
  if(bbox_set) {
    command_options.push_back(Oxs_TclCommandLineOption("minpt",3));
    command_options.push_back(Oxs_TclCommandLineOption("maxpt",3));
    command_options.push_back(Oxs_TclCommandLineOption("span",3));
    command_options.push_back(Oxs_TclCommandLineOption("relpt",3));
  } else {
    // bbox not set, so disable options that require it
    command_options.push_back(Oxs_TclCommandLineOption("minpt",0));
    command_options.push_back(Oxs_TclCommandLineOption("maxpt",0));
    command_options.push_back(Oxs_TclCommandLineOption("span",0));
    command_options.push_back(Oxs_TclCommandLineOption("relpt",0));
  }
  command_options.push_back(Oxs_TclCommandLineOption("rawpt",3));
  command_options.push_back(Oxs_TclCommandLineOption("scalars",
					     scalarfields.GetSize()));
  command_options.push_back(Oxs_TclCommandLineOption("vectors",
 ...
 ThreeVector& value) const
{
  ...
  cmd.GetResultListItem(0,value.x);
  cmd.GetResultListItem(1,value.y);
  cmd.GetResultListItem(2,value.z);
  if(norm_set) {
    if(norm == 1.0) {
      value.MakeUnit();
    } else {
