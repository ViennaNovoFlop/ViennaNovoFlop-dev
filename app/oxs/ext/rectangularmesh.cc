...
/////////////////////////////////////////////////////////////////////
// Oxs_RectangularMesh
void Oxs_RectangularMesh::InitScaling(const Oxs_Box& box)
 ...
 Oxs_OwnedPointer<Oxs_RectangularMesh>& out_mesh) const
{
  ...
  ThreeVector new_cellsize = cellsize;
  ...
  ThreeVector new_base = base;
  new_base.Accum(-0.5,cellsize);
  ...
  Oxs_RectangularMesh *refined_mesh
    = new Oxs_RectangularMesh(name,director,new_base,new_cellsize,
			      refine_x*xdim,refine_y*ydim,refine_z*zdim);

  out_mesh.SetAsOwner(refined_mesh);
}


void Oxs_RectangularMesh::GetBoundingBox(Oxs_Box& bbox) const
{
...
void Oxs_RectangularMesh::Center(OC_INDEX index,ThreeVector &location) const
{
#ifndef NDEBUG
  if(index>elementcount) {
    String msg = String("Index out of range "
      "(Oxs_RectangularMesh::Location(OC_INDEX) const)");
    throw Oxs_ExtError(msg.c_str());
  }
#endif
...
void Oxs_RectangularMesh::GetCenter(OC_INDEX index, ThreeVector& vec) const
{
#ifndef NDEBUG
  if(index>elementcount) {
    String msg = String("Index out of range "
...
OC_INDEX
 Oxs_RectangularMesh::FindNearestIndex(const ThreeVector& location) const
...
OC_BOOL
Oxs_RectangularMesh::GetNeighborPoint
...
OC_INDEX
Oxs_RectangularMesh::BoundaryList
...
{ // Boundary extraction.  Returns list (technically, an STL vector) of
  ...
  if(region_id<0) {
    String msg=String("Region name ")
      ...
      + String(" (Oxs_RectangularMesh::BoundaryList() in object")
      + String(InstanceName())
      + String(").");
...
///////////////////////////////////////////////////////////////////////////
//Print out boundary lists
///////////////////////////////////////////////////////////////////////////
//String m= String("Boundary 1 list: \n");
//vector<OC_INDEX>::iterator vit;
//for(vit=BoundaryIndexList.begin();vit!=BoundaryIndexList.end();vit++) {
//m.append(to_string(*vit)+"\n");
//}
////Just to get a non-zero value
//m.append(to_string(1.)+to_string(2.)+to_string(3.)+"\n");
//throw Oxs_Ext::Error(m.c_str());
//
///////////////////////////////////////////////////////////////////////////
...
void Oxs_RectangularMesh::WriteOvf
 ...
 ) const
{
  ...
  if(strcmp("rectangular",meshtype)!=0) {
    String msg=String("Unrecognized mesh type request: ")
      + String(meshtype)
      + String("(Oxs_RectangularMesh::WriteOvf() in object")
  ...
  if(headers) {
    try {
      ...
      if(Size()>0) {
        ...
        maxmag=sqrt(maxmag);
        minmag=sqrt(minmag);
        minmag*=0.9999;  // Underestimate lower bound by 0.01% to protect against
        /// rounding errors.
      }
      ...
      if(datawidth==4) {
        ...
        if(headers) {
          ...
          if(Vf_OvfFileFormatSpecs::WriteBinary(channel,buf,1)) {
            throw Oxs_RectangularMesh_WBError();
          }
        }
        OC_INDEX size=Size();
        for(OC_INDEX i=0 ; i<size ; ++i) {
  ...
  } catch(Oxs_RectangularMesh_WBError&) {
    OXS_EXTTHROW(Oxs_DeviceFull,
...
void Oxs_RectangularMesh::DumpGeometry
  ...
  if(type == vf_ovf20mesh_irregular) {
    ...
    if(!header.IsValidGeom()) {
      String msg=String("Invalid header (irregular mesh type) in"
                        " Oxs_RectangularMesh::DumpGeometry()"
                        " in object ")
        + String(InstanceName());
      throw Oxs_ExtError(msg.c_str());
    }
...
OC_BOOL Oxs_RectangularMesh::IsCompatible(const Vf_Mesh* vfmesh) const
{
...
void Oxs_RectangularMesh::FillMeshValueExact
 ...
 Oxs_MeshValue<ThreeVector>& vec) const
{
  ...
  // Both Oxs_RectangularMesh and Vf_GridVec3f access with the
  // x-dimension index changing fastest, z-dimension index chaning
  // slowest.
  OC_INDEX i,j,k;
...
// in that for Oxs_RectangularMesh all cells have same volume.
OC_REAL8m Oxs_RectangularMesh::VolumeSum
...
ThreeVector Oxs_RectangularMesh::VolumeSum
...
OC_REAL8m Oxs_RectangularMesh::VolumeSumXp
(const Oxs_MeshValue<OC_REAL8m>& scalar) const
{
  if(!scalar.CheckMesh(this)) {
    throw Oxs_ExtError(this,
...
ThreeVector Oxs_RectangularMesh::VolumeSumXp
(const Oxs_MeshValue<ThreeVector>& vec) const
{
  if(!vec.CheckMesh(this)) {
    throw Oxs_ExtError(this,
...
////////////////////////////////////////////////////////////////////////
// Max angle routine.  This routine returns the minimum dot
// product between neighboring vectors across the mesh, for all
// those vectors for which the corresponding entry in zero_check
// is non-zero.
//   If the input Oxs_MeshValue<ThreeVector> array contents are all
// unit vectors, then the max angle can be construed from acos(mindot).
// NOTE 1: These routines are likely sensitive to boundary conditions.
// NOTE 2: For nearly parallel vectors, the dot product does not return
//         very precise information about the angle between the vectors,
//         since cos(theta) = 1 - 0.5*theta^2 + ... is dominated by the
//         leading "1" term for theta<<1.
OC_REAL8m Oxs_RectangularMesh::MinNeighborDot
(const Oxs_MeshValue<ThreeVector>& vec,
 const Oxs_MeshValue<OC_REAL8m>& zero_check) const
{
  if(!vec.CheckMesh(this)) {
    throw Oxs_ExtError(this,
			 "Incompatible import array to"
			 " MinNeighborDot(const Oxs_MeshValue<ThreeVector>&,"
			 " const Oxs_MeshValue<OC_REAL8m>& scale)");
  }
  if(elementcount<1) return 0.0;
  if(elementcount<2) return vec[OC_INDEX(0)].MagSq(); // Not certain
  /// what most sensible return is in 1 cell case.  This seems as good
  /// as any.  If the input is suppose to be unit spins, then the caller
  /// might want to handle the 1 cell case itself.

  OC_REAL8m mindot1 = DBL_MAX;
  OC_REAL8m mindot2 = DBL_MAX;
  ...
  // some of the dot products by using a triangle inequality.
  ...
  // First check min dot's for x-neighbors
  if(xdim>1) {
    OC_INDEX index=0;
    ...
    for(row=0;row<row_count;row++) {
      ...
      for(x=1;x<xdim-1;x+=2) {
	// Here x-1, x and x+1 are always valid indices
	if(zero_check[index]!=0.0) {
	  ThreeVector temp = vec[index];
	  OC_REAL8m dot1 = temp*vec[index-1];
	  OC_REAL8m dot2 = temp*vec[index+1];
	  if(zero_check[index-1]!=0.0 && dot1<mindot1) mindot1=dot1;
	  if(zero_check[index+1]!=0.0 && dot2<mindot2) mindot2=dot2;
	}
	index+=2;
      }
	...
	if(zero_check[index+1]!=0.0) {
          ThreeVector temp = vec[index+1];
          if(zero_check[index]!=0.0) {
            OC_REAL8m dot1 = temp*vec[index];
            if(dot1<mindot1) mindot1=dot1;
          }
          if(zero_check[index+2]!=0.0) {
            OC_REAL8m dot2 = temp*vec[index+2];
            if(dot2<mindot2) mindot2=dot2;
          }
        }
	index+=2;
      }
	...
	/// one left over from the above x-loop.
	if(zero_check[index]!=0.0 && zero_check[index+1]!=0.0) {
	  OC_REAL8m dot1 =  vec[index]*vec[index+1];
	  if(dot1<mindot1) mindot1=dot1;
	}
  ...
  // Next check min dot's for y-neighbors
  if(ydim>1) {
	...
	for(x=0;x<xdim;x++) {
	  if(zero_check[index]!=0.0) {
	    ThreeVector temp = vec[index];
	    OC_REAL8m dot1 = temp*vec[index-xdim];
	    OC_REAL8m dot2 = temp*vec[index+xdim];
	    if(zero_check[index-xdim]!=0.0 && dot1<mindot1) mindot1=dot1;
	    if(zero_check[index+xdim]!=0.0 && dot2<mindot2) mindot2=dot2;
	  }
	  index++;
	}
	...
	for(x=0;x<xdim;x++) {
	  if(zero_check[index]!=0.0 && zero_check[index-xdim]!=0.0) {
	    OC_REAL8m dot1 = vec[index]*vec[index-xdim];
	    if(dot1<mindot1) mindot1=dot1;
	  }
	  index++;
	}
  ...
  // Finally check min dot's for z-neighbors
  if(zdim>1) {
	...
	if(zero_check[index]!=0.0) {
	  ThreeVector temp = vec[index];
	  OC_REAL8m dot1 = temp*vec[index-xydim];
	  OC_REAL8m dot2 = temp*vec[index+xydim];
	  if(zero_check[index-xydim]!=0.0 && dot1<mindot1) mindot1=dot1;
	  if(zero_check[index+xydim]!=0.0 && dot2<mindot2) mindot2=dot2;
	}
	++index;
      }
    }
	...
	if(zero_check[index]!=0.0 && zero_check[index+xydim]!=0.0) {
	  OC_REAL8m dot2 = vec[index]*vec[index+xydim];
	  if(dot2<mindot2) mindot2=dot2;
	}
	++index;
      }
  ...
  OC_REAL8m mindot = mindot1;
  if(mindot2<mindot) mindot=mindot2;

  if(mindot == DBL_MAX) mindot=0.0; // No valid (non-zero) vec's.
