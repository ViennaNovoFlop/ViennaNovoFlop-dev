    ...
    coef_size(0), coef(NULL), mesh_id(0)
{
  ...
  if(coef_size>max_coef_size) {
      ...
      Oc_Snprintf(buf,sizeof(buf),
		  "Oxs_Atlas object %s has too many regions: %lu"
                  " (max allowed: %lu)",
                  atlas->InstanceName(),
  ...
  if(params.size()%3!=0) {
      char buf[4096];
      Oc_Snprintf(buf,sizeof(buf),
		  "Number of elements in %s sub-list must be"
		  " divisible by 3"
  ...
  try {
    ...
    for(OC_INT4m ip=0;static_cast<size_t>(ip)<params.size();ip+=3) {
      ...
      if(i1<0 || i2<0) {
        ...
        if(i1<0) {
          Oc_Snprintf(buf,sizeof(buf),
                      "First entry in %s[%ld] sub-list, \"%s\","
                      " is not a known region in atlas \"%s\".  ",
                      typestr.c_str(),long(ip/3),params[ip].c_str(),
                      atlas->InstanceName());
          cptr += strlen(buf);
        }
        if(i2<0) {
          Oc_Snprintf(cptr,sizeof(buf)-(cptr-buf),
                      "Second entry in %s[%ld] sub-list, \"%s\","
                      " is not a known region in atlas \"%s\".  ",
                      typestr.c_str(),long(ip/3),params[ip+1].c_str(),
                      atlas->InstanceName());
        }
      ...
      if(err) {
        char buf[4096];
        Oc_Snprintf(buf,sizeof(buf),
                    "Third entry in %s[%ld] sub-list, \"%s\","
                    " is not a valid floating point number.",
                    typestr.c_str(),long(ip/3),params[ip+2].c_str());
        throw Oxs_ExtError(this,buf);
      }
      coef[i1][i2]=coefpair;
 ...
 ) const
{
  ...
  if(mesh==NULL) {
    String msg = String("Import mesh to Oxs_Exchange6Ngbr::GetEnergyA()"
                        " routine of object ")
      + String(InstanceName())
 ...
 ) const
{
  ...
  const Oxs_RectangularMesh* mesh
    = dynamic_cast<const Oxs_RectangularMesh*>(state.mesh);
  if(mesh==NULL) {
    String msg = String("Import mesh to Oxs_Exchange6Ngbr::GetEnergyLex()"
                        " routine of object ")
      + String(InstanceName())
      + String(" is not an Oxs_RectangularMesh object.");
