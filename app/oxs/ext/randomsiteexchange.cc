  ...
  for(OC_INDEX z=0;z<zdim;z++) {
	...
	for(OC_INDEX li=0;li<3;li++) { // Link index: 0=x, 1=y, 2=z
	  ...
	  if(luck<=linkprob) {
	    ...
	    OC_REAL8m wgt=0.;
	    switch(li) {
	      case 0:  if(x+1<xdim) offset=1;      wgt=wgtx;  break;
	      case 1:  if(y+1<ydim) offset=xdim;   wgt=wgty;  break;
	      default: if(z+1<zdim) offset=xydim;  wgt=wgtz;  break;
	    }
	    if(offset>0) {
 ...
 ) const
{
  ...
  if(mesh==NULL) {
    String msg =
      String("Import mesh to Oxs_RandomSiteExchange::GetEnergy()"
             " routine of object ")
      + String(InstanceName())
