 ...
 ) const
{
  ...
  if(mesh==NULL) {
    String msg = String("Import mesh to Oxs_ExchangePtwise::GetEnergy()"
                        " routine of object ")
      + String(InstanceName())
    ...
    for(OC_INDEX y=0;y<ydim;++y) {
      for(OC_INDEX x=0;x<xdim;++x) {
        ...
        for(OC_INDEX dir=0;dir<3;++dir) { // 0=>x, 1=>y, 2=>z
          ...
          OC_REAL8m wgt=0.;
          switch(dir) {
          case 0:  if(x+1<xdim) { j=i+1;     wgt=wgtx; } break;
          case 1:  if(y+1<ydim) { j=i+xdim;  wgt=wgty; } break;
          default: if(z+1<zdim) { j=i+xydim; wgt=wgtz; } break;
          }
          if(j==0) continue; // Edge case; skip
          OC_REAL8m Aj=A[j];
          if(Aj == 0.0) continue;
