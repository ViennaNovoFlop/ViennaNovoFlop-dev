...
void PBC_Demag_2D::LoadPbcDemagTensor(
    ...
    if (tensor_file_name.length() > 0) {
        ...
        // Npbc_diag
        vffi = Vf_FileInput::NewReader(diagname.c_str());
        ...
        if (vffi != NULL) {
            ...
            if (file_mesh == NULL || !IsCompatible(file_mesh)) {
                if (file_mesh != NULL) delete file_mesh;
            } else {

                FillMeshValueExact(file_mesh, Npbc_diag);
                delete file_mesh;

                // Npbc_offdiag
                vffi = Vf_FileInput::NewReader(offdiagname.c_str());
                if (vffi != NULL) {
                    file_mesh = vffi->NewMesh();
                    ...
                    if (file_mesh == NULL || !IsCompatible(file_mesh)) {
                        ...
                        FillMeshValueExact(file_mesh, Npbc_offdiag);
...
//copy from rectangularmesh.cc of the OOMMF codes
//(OOMMF release 1.2.0.4, snapshot 2009.12.16)

BOOL PBC_Demag_2D::IsCompatible(const Vf_Mesh* vfmesh) const {
    const Vf_GridVec3f* gridmesh
            = dynamic_cast<const Vf_GridVec3f*> (vfmesh);
    if (gridmesh == NULL) return 0;

    INT4m isize, jsize, ksize;
    gridmesh->GetDimens(isize, jsize, ksize);
    if (static_cast<INT4m> (xdim) != isize ||
            static_cast<INT4m> (ydim) != jsize ||
            static_cast<INT4m> (zdim) != ksize) {
        return 0;
    }
...
//modified from rectangularmesh.cc of the OOMMF codes
//(OOMMF release 1.2.0.4, snapshot 2009.12.16)

void PBC_Demag_2D::FillMeshValueExact
(const Vf_Mesh* vfmesh,
        Oxs_MeshValue<ThreeVector>& vec) const {
    const Vf_GridVec3f* gridmesh
            = dynamic_cast<const Vf_GridVec3f*> (vfmesh);
    if (gridmesh == NULL || !IsCompatible(vfmesh)) {
        String msg = String("Incompatible Vf_Mesh import to"
    ...
    REAL8m scale = gridmesh->GetValueMultiplier();
    UINT4m index = 0;

    if (scale == 1.0) { // Common case?
        for (k = 0; k < zdim; ++k) for (j = 0; j < ydim; ++j) for (i = 0; i < xdim; ++i) {
                    const Nb_Vec3<REAL8>&nbvec = gridmesh->GridVec(i, j, k);
                    index = xdim * ydim * k + xdim * j + i;
                    vec[index].Set(nbvec.x, nbvec.y, nbvec.z);
                }
    } else {
        for (k = 0; k < zdim; ++k) for (j = 0; j < ydim; ++j) for (i = 0; i < xdim; ++i) {
                    const Nb_Vec3<REAL8>&nbvec = gridmesh->GridVec(i, j, k);
                    index = xdim * ydim * k + xdim * j + i;
                    vec[index].Set(scale * nbvec.x,
                            scale * nbvec.y,
                            scale * nbvec.z);
                }
    }
