...
void Oxs_SetErrorCode(Tcl_Interp *interp, const char* value)
{
  ...
  Tcl_SetErrorCode(interp, Oc_AutoBuf(value).GetStr(), NULL);
#else
		       ...
		       int argc,CONST84 char** argv)
{
  ...
  try {
    ...
    error_code = Oxs_MergeList(ecvec);
    ...
    // Some routines in the pkg area throw bare char*'s or
    // strings.  These should be changed to Oc_Exceptions.
    error_message =
      String("Error thrown from inside"
...
{ // On success, returns a list of events with associated state id
  ...
  if(!results.empty()) {
    ...
    whole_list = Oxs_MergeList(event_list);
  }
 ...
 int argc,CONST84 char** argv)
{
  ...
  String regions_str = Oxs_MergeList(regions);
