...
#define OXS_GET_EXT_OBJECT(params,subtype,obj)                        \
   { Oxs_OwnedPointer<Oxs_Ext> _goeo_tmp;                             \
     ...
     if(obj.GetPtr()==NULL) {                                         \
        char buf[1024];                                               \
        Oc_Snprintf(buf,sizeof(buf),"Failed downcast: %s is not of"   \
                    " type %s in source code file %s, line %s.",      \
...
#define OXS_GET_INIT_EXT_OBJECT(label,subtype,obj)                    \
   { Oxs_OwnedPointer<Oxs_Ext> _goeo_tmp;                             \
     ...
     if(obj.GetPtr()==NULL) {                                         \
        char buf[1024];                                               \
        Oc_Snprintf(buf,sizeof(buf),"Error processing label \"%s\""   \
                    " --- Failed downcast: %s is not of type %s"      \
                    " in source code file %s, line %s.",              \
