...
(__m128d& tx,__m128d& ty,__m128d& tz)
{
  ...
  OC_REAL8 smagsq0,smagsq1;
  _mm_storel_pd(&smagsq0,tmagsq);
  _mm_storeh_pd(&smagsq1,tmagsq);

  if(smagsq0 <= 0.0 || smagsq1 <= 0.0) { // Special case handling
    ThreeVector t0,t1;
    if(smagsq0 <= 0.0) {
      t0.Random(1.0);
    } else {
  ...
  OC_SSE_MASK absmask;
  absmask.imask = 0x7FFFFFFFFFFFFFFF;
 ...
 OC_REAL8m* magsq1)
{
  ...
  if(magsq0) *magsq0 = smagsq0;
  if(magsq1) *magsq1 = smagsq1;
