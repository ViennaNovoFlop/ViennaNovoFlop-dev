...
{ // Sets micromag problem to initial state.
  ...
  {
    char buf[1024];
    Oc_Snprintf(buf,sizeof(buf),"%s InitRandom",mif_object.c_str());
...
{ 
  ...
  OC_REAL8m mindot = DBL_MAX;
  for(OC_INT4m i=0;i<count;i++) {
    mindot = state->mesh->MinNeighborDot(state->spin,*(state->Ms));
  }
  if(mindot >  1 - 8*OC_REAL8_EPSILON) mindot =  1;
  if(mindot < -1 + 8*OC_REAL8_EPSILON) mindot = -1;
  char buf[256];
  Oc_Snprintf(buf,sizeof(buf),"%.17g",
              static_cast<double>(acos(mindot)*(180.0/PI)));
  out = String(buf);
  return 0;
}
#endif // DEVELOP_TEST
