      ...
      start_last_timestep(0.)
{
  ...
  if(HasInitValue("projection_options")) {
    ...
    if(pieces.Split(projopts)!=TCL_OK) {
      ...
      Oc_Snprintf(temp_buf,sizeof(temp_buf),
                  ...
                  "block is not a proper Tcl list: %s",
                  projopts.c_str());
	...
	Oc_Snprintf(temp_buf,sizeof(temp_buf),
		    "Format error in projection_options subblock---"
		    "label \"%s\" appears more than once.",
		    pieces[item]);
	throw Oxs_ExtError(this,temp_buf);
      }
  ...
  if(HasInitValue("projection_outputs")) {
	...
	Oxs_SplitList tmpparams;
	if(tmpparams.Split(pieces[i+1])!=TCL_OK) {
          char temp_buf[4096];
          Oc_Snprintf(temp_buf,sizeof(temp_buf),
                      "Format error in projection_outputs subblock---"
                      "vector field spec for output %s is not a proper"
                      " Tcl list: %s",
                      pieces[i],pieces[i+1]);
          throw Oxs_ExtError(this,temp_buf);
        }
	...
	if(it != projection_options.end()) {
	  Oxs_SplitList tmpoptions;
	  if(tmpoptions.Split(it->second)!=TCL_OK) {
            char temp_buf[4096];
            Oc_Snprintf(temp_buf,sizeof(temp_buf),
                        "Format error in projection_options subblock---"
                        "options for output %s is not a proper"
                        " Tcl list: %s",
	  ...
	  if(tmpoptions.Count()%2!=0) {
	    ...
	    Oc_Snprintf(temp_buf,sizeof(temp_buf),
			...
			"with name \"%s\" has %d elements, which is "
			"not even.",po.name.c_str(),tmpoptions.Count());
	    throw Oxs_ExtError(this,temp_buf);
	  }
	  for(int j=0;j<tmpoptions.Count();j+=2) {
		...
		Oc_Snprintf(temp_buf,sizeof(temp_buf),
			    "Format error in projection_options subblock---"
			    "\"normalize\" option value \"%s\" associated"
			    " with name \"%s\" is not an integer.",
			    tmpoptions[j+1],po.name.c_str());
		throw Oxs_ExtError(this,temp_buf);
	      }
		...
		Oc_Snprintf(temp_buf,sizeof(temp_buf),
			    "Format error in projection_options subblock---"
			    "\"scaling\" option value \"%s\" associated with"
			    " name \"%s\" is not a floating point value.",
			    tmpoptions[j+1],po.name.c_str());
		throw Oxs_ExtError(this,temp_buf);
	      }
	      ...
	      Oc_Snprintf(temp_buf,sizeof(temp_buf),
			  "Format error in projection_options subblock---"
			  "option \"%s\" associated with name \"%s\" "
			  "is unknown.",
			  tmpoptions[j],po.name.c_str());
	      throw Oxs_ExtError(this,temp_buf);
	    }
	  }
...
OC_BOOL Oxs_Driver::Init()
{
  ...
  if(arglist.Count()!=2) {
    ...
    String precise_format = Oxs_MergeList(&sarr);
    ...
    if(checkpoint_interval>=0.0
      ...
      if(double(wall_time)>=checkpoint_interval) {
...
#if 1 // Old-style warning messages
        static Oxs_WarningMessage foo(4);
        ...
        String msg = String("Error saving checkpoint file \"")
          + String(full_checkname) + String("\"");
        checkpoint_id = current_state.GetPtr()->Id();
        Nb_DString tmpchknam
          = Nb_TempName(checkpoint_file.c_str(),".tmp",".");
        try {
	  ...
	  ++checkpoint_writes;
          ...
          // Checkpoint write was successful.  If we previously had
          // checkpoint write failures, and had enough of them to disable
          // the associated warning message, then re-enable the warning
          // message for one additional warning.
          if(foo.GetCurrentCount() >= foo.GetMaxCount()) {
            foo.SetCurrentCount(foo.GetMaxCount() - 1);
          }
          ...
          String tmpmsg = String("Error saving checkpoint file \"")
          ...
          msg += String(" --- ") + String(errmsg);
          foo.Send(revision_info,OC_STRINGIFY(__LINE__),msg.c_str());
        } catch (...) {
          foo.Send(revision_info,OC_STRINGIFY(__LINE__),msg.c_str());
        }
        Oc_Times(cpu_time,checkpoint_time); // Reset checkpoint time
#else // New-style warning messages
        Oc_AutoBuf full_checkname;  // Full path to checkpoint file
        Oc_DirectPathname(checkpoint_file.c_str(),full_checkname);
        checkpoint_id = current_state.GetPtr()->Id();
        Nb_DString tmpchknam
	  ...
	  ++checkpoint_writes;
          ...
          String msg = String("Error saving checkpoint file \"")
            ...
            + String("\n   ar line:") + String(OC_STRINGIFY(__LINE__));
          throw msg.c_str();
        }
        ...
        try {
          ...
          String prefix = String("Error saving checkpoint file \"")
            + String(full_checkname) + String("\" --- ");
          err.PrependMessage(prefix.c_str());
          throw;
        }
...
void Oxs_Driver::UpdateSpinAngleData(const Oxs_SimState& state) const
{
  ...
  if(state.mesh->Size()<2) maxang = 0.0; // Special case
  else {
    OC_REAL8m mindot = state.mesh->MinNeighborDot(state.spin,*(state.Ms));
