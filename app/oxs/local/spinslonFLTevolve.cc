...
OXS_EXT_REGISTER(Oxs_SpinSlonFLTEvolve);

/* End includes */

// Constructor
Oxs_SpinSlonFLTEvolve::Oxs_SpinSlonFLTEvolve(
...
//fprintf(f,"Flag #1\n");
  ...
  if(HasInitValue("epsprime")){
    OXS_GET_INIT_EXT_OBJECT("epsprime",Oxs_ScalarField,epsprime_init);
  } else {
  	  if(HasInitValue("alpha")) {
          OXS_GET_INIT_EXT_OBJECT("alpha",Oxs_ScalarField,epsprime_init);
          } else {
          epsprime_init.SetAsOwner(dynamic_cast<Oxs_ScalarField *>
                          (MakeNew("Oxs_UniformScalarField",director,
                                   "value 0.01")));//default alpha 0.5
          }
  }
  ...
  if(method.compare("rk2")==0) {
    rkstep_ptr = &Oxs_SpinSlonFLTEvolve::TakeRungeKuttaStep2;
  } else if(method.compare("rk2heun")==0) {
    rkstep_ptr = &Oxs_SpinSlonFLTEvolve::TakeRungeKuttaStep2Heun;
  } else if(method.compare("rk4")==0) {
    rkstep_ptr = &Oxs_SpinSlonFLTEvolve::TakeRungeKuttaStep4;
  } else if(method.compare("rkf54m")==0) {
    rkstep_ptr = &Oxs_SpinSlonFLTEvolve::TakeRungeKuttaFehlbergStep54M;
  } else if(method.compare("rkf54s")==0) {
    rkstep_ptr = &Oxs_SpinSlonFLTEvolve::TakeRungeKuttaFehlbergStep54S;
  } else if(method.compare("rkf54")==0) {
    rkstep_ptr = &Oxs_SpinSlonFLTEvolve::TakeRungeKuttaFehlbergStep54;
  } else {
    throw Oxs_ExtError(this,"Invalid initialization detected:"
  ...
  max_dm_dt_output.Setup(this,InstanceName(),"Max dm/dt","deg/ns",0,
     &Oxs_SpinSlonFLTEvolve::UpdateDerivedOutputs);
  ...
  ave_J_output.Setup(this,InstanceName(),"average J","A/m^2",0,
     &Oxs_SpinSlonFLTEvolve::UpdateSpinTorqueOutputs);
...
OC_BOOL Oxs_SpinSlonFLTEvolve::Init()
{
  ...
  // Free memory used by LLG damping parameter alpha, epsprime, gamma etc.
  alpha.Release();
  epsprime.Release();
  gamma.Release();
  g.Release();
...
void Oxs_SpinSlonFLTEvolve::UpdateMeshArrays(const Oxs_Mesh* mesh)
{
  mesh_id = 0; // Mark update in progress
  const Oxs_RectangularMesh* rmesh
    = dynamic_cast<const Oxs_RectangularMesh*>(mesh);
  if(rmesh==NULL) {
    String msg="Import mesh to Oxs_SpinSlonFLTEvolve::UpdateMeshArrays"
...
OC_REAL8m Oxs_SpinSlonFLTEvolve::EvaluateJProfileScript
(OC_UINT4m stage,
...
OC_REAL8m
Oxs_SpinSlonFLTEvolve::PositiveTimestepBound
(OC_REAL8m max_dm_dt)
...
void Oxs_SpinSlonFLTEvolve::Calculate_dm_dt
...
// Slonczewski function g(m.p) for dielectric
      ...
      mdotp *= polarization[i];
      mdotp += 1.;
      OC_REAL8m g_func = 0.5 * polarization[i];
      g_func /= mdotp;
...
// starting to assemble the complete term -> g_func*[alpha*mxp-mxmxp] in the case of non vanishing voltage epsprime!!!
      ThreeVector scratchA = mxp;
      scratchA *= cell_epsprime;
      scratchA -= mxmxp;
      scratchA *= g_func;
/////////////////////////////////////////////////////////////////////
...
void Oxs_SpinSlonFLTEvolve::CheckCache(const Oxs_SimState& cstate)
{
  ...
  if(!cache_good) {
    throw Oxs_ExtError(this,
       "Oxs_SpinSlonFLTEvolve::CheckCache: Invalid data cache.");
  }
}


void
Oxs_SpinSlonFLTEvolve::AdjustState
(OC_REAL8m hstep,
 OC_REAL8m mstep,
 ...
 OC_REAL8m& norm_error) const
{
  ...
  // to Oxs_SpinSlonFLTEvolve::Step() takes 2 half-steps, and it is the
  ...
  // in by the Oxs_SpinSlonFLTEvolve::NegotiateTimeStep() method.
}


void Oxs_SpinSlonFLTEvolve::UpdateTimeFields
...
void Oxs_SpinSlonFLTEvolve::NegotiateTimeStep
...
{ // This routine negotiates with driver over the proper step size.
  ...
  if(!cstate.GetDerivedData("Max dm/dt",max_dm_dt)) {
    throw Oxs_ExtError(this,
       "Oxs_SpinSlonFLTEvolve::NegotiateTimeStep: max_dm_dt not cached.");
  }
  ...
  if(!cstate.GetDerivedData("Timestep lower bound",
                            timestep_lower_bound)) {
    throw Oxs_ExtError(this,
       "Oxs_SpinSlonFLTEvolve::NegotiateTimeStep: "
...
OC_BOOL
Oxs_SpinSlonFLTEvolve::CheckError
...
void Oxs_SpinSlonFLTEvolve::TakeRungeKuttaStep2
(OC_REAL8m stepsize,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
    ...
    throw Oxs_ExtError(this,
                         "Oxs_SpinSlonFLTEvolve::TakeRungeKuttaStep2:"
...
void Oxs_SpinSlonFLTEvolve::TakeRungeKuttaStep2Heun
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
     ...
     !endstate->AddDerivedData("dE/dt",dE_dt)) {
    throw Oxs_ExtError(this,
                         "Oxs_SpinSlonFLTEvolve::TakeRungeKuttaStep2Heun:"
                         " Programming error; data cache already set.");
  }
...
void Oxs_SpinSlonFLTEvolve::TakeRungeKuttaStep4
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_SpinSlonFLTEvolve::RungeKuttaFehlbergBase54
(RKF_SubType method,
 OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
...
{ // Runge-Kutta-Fehlberg routine with combined 4th and 5th
  ...
  default:
    throw Oxs_ExtError(this,
                 "Oxs_SpinSlonFLTEvolve::RungeKuttaFehlbergBase54:"
...
void Oxs_SpinSlonFLTEvolve::TakeRungeKuttaFehlbergStep54
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_SpinSlonFLTEvolve::TakeRungeKuttaFehlbergStep54M
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_SpinSlonFLTEvolve::TakeRungeKuttaFehlbergStep54S
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
OC_REAL8m Oxs_SpinSlonFLTEvolve::MaxDiff
...
void Oxs_SpinSlonFLTEvolve::AdjustStepHeadroom(OC_INT4m step_reject)
...
OC_BOOL
Oxs_SpinSlonFLTEvolve::Step(const Oxs_TimeDriver* driver,
                      Oxs_ConstKey<Oxs_SimState> current_state_key,
                      ...
                      Oxs_Key<Oxs_SimState>& next_state_key)
{
  ...
  if(next_timestep<=0.0 ||
    ...
    if(cstate.stage_number==0
      ...
      throw Oxs_ExtError(this,
           "Oxs_SpinSlonFLTEvolve::Step; Programming error:"
  ...
  // -mjd, 21-July-2004
  {
    ...
    if(!cstate.GetDerivedData("dE/dt",current_dE_dt)) {
      throw Oxs_ExtError(this,"Oxs_SpinSlonFLTEvolve::Step:"
...
void Oxs_SpinSlonFLTEvolve::UpdateDerivedOutputs(const Oxs_SimState& state)
{ // This routine fills all the Oxs_SpinSlonFLTEvolve Oxs_ScalarOutput's to
...
void Oxs_SpinSlonFLTEvolve::UpdateSpinTorqueOutputs(const Oxs_SimState& state)
{
...
// starting to assemble the complete term -> g_func*[alpha*mxp-mxmxp] --> non vanishing voltage load epsprime
      ThreeVector scratchA = mxp;
      scratchA *= cell_epsprime;
      scratchA -= mxmxp;
      scratchA *= g_func;
