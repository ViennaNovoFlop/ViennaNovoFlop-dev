		...
		has_tempscript(0)
{
  ...
  if(HasInitValue("tempscript")) {
		...
		tempscript_opts.push_back(Oxs_TclCommandLineOption("stage",1));
...
{ // Imports: mesh_, Ms_, mxH_, spin_, pE_pt_
  ...
  const OC_UINT4m size = mesh_.Size(); // Assume all imports are compatible
		      ...
		      Oxs_Key<Oxs_SimState>& next_state)
{
  ...
  OC_UINT4m size,i; // Mesh size and indexing variable
