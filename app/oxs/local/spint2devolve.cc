...
OXS_EXT_REGISTER(Anv_SpinT2DEvolve);

/* End includes */

// Constructor
Anv_SpinT2DEvolve::Anv_SpinT2DEvolve(
  const char* name,     // Child instance id
  Oxs_Director* newdtr, // App director
  const char* argstr)   // MIF input block parameters
  : Oxs_TimeEvolver(name,newdtr,argstr),
    mesh_id(0),has_u_profile(0),
...
// getting/checking  J for current direction; twindbach 14.06.2012
  if(HasInitValue("j")) {
   OXS_GET_INIT_EXT_OBJECT("j",Oxs_VectorField,j_init);
  } else {
    char buf[4096];
    Oc_Snprintf(buf,sizeof(buf),"SpinT2DEvolve: no j-vector (current direction given)");
    throw Oxs_Ext::Error(this,buf);
  }

// checking if second order gradient is set
  gradient_order = GetIntInitValue("gradient_order",1);
// for debugging
//  if (gradient_order!=1 || gradient_order!=2 ) {
//    char buf[4096];
//    Oc_Snprintf(buf,sizeof(buf),"SpinT2DEvolve: gradient_order was set to %i.\n This is not valid - only 1 or 2 are allowed",gradient_order);
//    throw Oxs_Ext::Error(this,buf);
//   }
	if(HasInitValue("u")) {
  ...
  if(method.compare("rk2")==0) {
    rkstep_ptr = &Anv_SpinT2DEvolve::TakeRungeKuttaStep2;
  } else if(method.compare("rk4")==0) {
    rkstep_ptr = &Anv_SpinT2DEvolve::TakeRungeKuttaStep4;
  } else if(method.compare("rkf54m")==0) {
    rkstep_ptr = &Anv_SpinT2DEvolve::TakeRungeKuttaFehlbergStep54M;
  } else if(method.compare("rkf54s")==0) {
    rkstep_ptr = &Anv_SpinT2DEvolve::TakeRungeKuttaFehlbergStep54S;
  } else if(method.compare("rkf54")==0) {
    rkstep_ptr = &Anv_SpinT2DEvolve::TakeRungeKuttaFehlbergStep54;
  } else {
    throw Oxs_Ext::Error(this,"Invalid initialization detected:"
  ...
  max_dm_dt_output.Setup(this,InstanceName(),"Max dm/dt","deg/ns",0,
     &Anv_SpinT2DEvolve::UpdateDerivedOutputs);
  ...
  ave_u_output.Setup(this,InstanceName(),"average u","m/s",0,
		 &Anv_SpinT2DEvolve::UpdateSpinTorqueOutputs);
...
OC_BOOL Anv_SpinT2DEvolve::Init()
{
  // Setup outputs
...
void Anv_SpinT2DEvolve::UpdateMeshArrays(const Oxs_Mesh* mesh)
{
  ...
  if(rmesh==NULL) {
    String msg="Import mesh to Anv_SpinT2DEvolve::UpdateMeshArrays"
      " is not an Oxs_RectangularMesh object.";
    throw Oxs_Ext::Error(msg.c_str());
  }

//extension to y and z, 14.06.2012; twindbach
  ...
  // Zero spin torque on fixed spins
	const OC_UINT4m size = mesh->Size();
	OC_UINT4m i,j;
  ave_u = 0.0;
  for(i=0;i<size;i++) {
    ave_u += u[i];
...
OC_REAL8m Anv_SpinT2DEvolve::EvaluateuProfileScript
(OC_UINT4m stage,
 ...
 OC_REAL8m total_time) const
{
  ...
  int index;
  if((index = u_profile_opts[0].position)>=0) { // stage
    u_profile_cmd.SetCommandArg(index,stage);
  }
  if((index = u_profile_opts[1].position)>=0) { // stage_time
    u_profile_cmd.SetCommandArg(index,stage_time);
  }
  if((index = u_profile_opts[2].position)>=0) { // total_time
    u_profile_cmd.SetCommandArg(index,total_time);
  }

  u_profile_cmd.SaveInterpResult();
  u_profile_cmd.Eval();
  if(u_profile_cmd.GetResultListSize()!=1) {
    String msg
      = String("Return script value is not a single scalar: ")
      + u_profile_cmd.GetWholeResult();
...
void Anv_SpinT2DEvolve::Calculate_dm_dt
...
{ // Imports: state, mxH_, pE_pt_
  ...
  const OC_UINT4m size = mesh->Size(); // Assume import data are compatible
  ...
  for(k=0;k<fixed_count;k++) {
    dm_dt_[GetFixedSpin(k)].Set(0.,0.,0.);
  }
  ...
  for(i=0;i<size;i++) {
    ...
    if(Ms_[i]==0) {
      dm_dt_[i].Set(0.0,0.0,0.0); 
                        ...
                        ThreeVector current_direction = j[i];
			OC_UINT4m x = 0;
                        OC_UINT4m y = 0;
                        OC_UINT4m z = 0;
                       //sorting out position of nodes
                        z=i/(n_x*n_y);
                        y=i%(n_x*n_y)/n_x;
                        x=i%n_x;
...
if(current_direction.x !=0.0) {
			...
			if(x>0) {
				k = i-1;
				if(Ms_[k]!=0.0) {
					gradx_m = 0.5*(base -spin_[k]);
				}
			}
			if(x<n_x-1) {
				k = i+1;
				if(Ms_[k]!=0.0) {
					gradx_m += 0.5*(spin_[k] - base);
				}
			}
			if(n_x>2) { // Free boundary correction
				if(x==0) {
					k = i+1;
					if(Ms_[k]!=0.0) {
						gradx_m += 0.5*(spin_[k] - base);
			...
			if(n_x>4) { // Free boundary correction
				if(x==0) {
					k = i+1;
					if(Ms_[k]!=0.0) {
						gradx_m = 12.0*(spin_[k]-base);
					}
				}
				if(x==1) {
					k = i+1;
					if(Ms_[k]!=0.0) { gradx_m  = 12.0*0.5*(spin_[k]-base);}
                                        k = i-1;  
                                        if(Ms_[k]!=0.0) { gradx_m += 12.0*0.5*(base-spin_[k]);}
					}
				if(x==n_x-1) {
					k = i-1;
					if(Ms_[k]!=0.0) {
						gradx_m = 12.0*(base-spin_[k]);
					}
				}
				if(x==n_x-2) {
					k = i+1;
			...
			gradx_m *= (1/(12.0*Xstep));
                       }

			gradx_m *= u[i];// gradx_m=u*pm_px
			gradx_m *= umult;
			gradx_m *= current_direction.x; // projection to x-axis
			m_gradx_m = spin_[i];
			m_gradx_m ^= gradx_m; // u*m^pm_px

			m_m_gradx_m = m_gradx_m;
			m_m_gradx_m ^= spin_[i]; //u*(m^pm_px)^m=-u*m^(m^pm_px)=u*pm_px

			scratch2 = m_m_gradx_m;
			scratch2 *= (1+cell_alpha*beta)/(1+cell_alpha*cell_alpha);
			dm_dt_[i] -= scratch2;   //dm_dt= -gamma_LL*m^H-alpha*gamma_LL*m^(m^H) -(1+alpha*beta)/(1+alpha^2)*u*pm_px
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// -------------------- Section for y-dependence ------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////

			ThreeVector grady_m(0.,0.,0.);
			ThreeVector m_grady_m(0.,0.,0.);
			ThreeVector m_m_grady_m(0.,0.,0.);

if(current_direction.y != 0.0) {
                       if (gradient_order == 1) {
// -------------------- here the calculation of u*grady_m begins ---------------------------------------------------
			if(y>0) {
				k = x+(y-1)*n_x+z*n_x*n_y;
				if(Ms_[k]!=0.0) {
					grady_m = 0.5*(base -spin_[k]);
				}
			}
			if(y<n_y-1) {
				k = x+(y+1)*n_x+z*n_x*n_y;
				if(Ms_[k]!=0.0) {
					grady_m += 0.5*(spin_[k] - base);
				}
			}
			...
			if(n_y>4) { // Free boundary correction
				if(y==0) {
					k = x+(y+1)*n_x+z*n_x*n_y;
					if(Ms_[k]!=0.0) {
						grady_m = 12.0*(spin_[k]-base);
					}
				}
				if(y==1) {
					k = x+(y+1)*n_x+z*n_x*n_y;
					if(Ms_[k]!=0.0) { grady_m  = 12.0*0.5*(spin_[k]-base);}
                                        k = x+(y-1)*n_x+z*n_x*n_y;  
                                        if(Ms_[k]!=0.0) { grady_m += 12.0*0.5*(base-spin_[k]);}
					}
				if(y==n_y-1) {
					k = x+(y-1)*n_x+z*n_x*n_y;
					if(Ms_[k]!=0.0) {
						grady_m = 12.0*(base-spin_[k]);
					}
				}
				if(y==n_y-2) {
					k = x+(y+1)*n_x+z*n_x*n_y;
			...
			grady_m *= (1/(12.0*Ystep));
                      }
		
			grady_m *= u[i];// gradx_m=u*pm_px; gradient of m is multiplied by the absolute value of u
			grady_m *= umult;// this takes care of time profiles e.g. step functions etc.
                        grady_m *= current_direction.y; // projection to y-axis

			m_grady_m = spin_[i];
			m_grady_m ^= grady_m; // u*m^pm_px

			m_m_grady_m = m_grady_m;
			m_m_grady_m ^= spin_[i]; //u*(m^pm_px)^m=-u*m^(m^pm_px)=u*pm_px

			scratch2 = m_m_grady_m;
			scratch2 *= (1+cell_alpha*beta)/(1+cell_alpha*cell_alpha);
			dm_dt_[i] -= scratch2;   //dm_dt= -gamma_LL*m^H-alpha*gamma_LL*m^(m^H) -(1+alpha*beta)/(1+alpha^2)*u*pm_px
...
//let's assume the terms will add up ...
      if(dm_dt_sq>0)	{
				inter_dEdt -= mxH;
				inter_dEdt *=fabs(cell_alpha*cell_mgamma); // inter_dEdt=-alpha*|gamma_LL|*m^H
if (current_direction.x != 0.0) {
// first for x-direction
				m_m_gradx_m *= (cell_alpha-beta)/(1+cell_alpha*cell_alpha); //m_m_gradx_m=(beta-alpha)/(1+alpha^2)*u*m^(m^pm_px)
				inter_dEdt -= m_m_gradx_m; //there was an error of sign here
				m_gradx_m *= (cell_alpha*beta+1)/(1+cell_alpha*cell_alpha); //m_gradx_m=(1+alpha*beta)/(1+alpha^2)*u*m^pmpx
				inter_dEdt += m_gradx_m; //inter_dEdt=-alpha*|gamma_LL|*m^H+(beta-alpha)/(1+alpha^2)*u*pm_px+(1+alpha*beta)/(1+alpha^2)*u*m^pmpx
}
if (current_direction.y !=0.0) {
// the for y-direction
				m_m_grady_m *= (cell_alpha-beta)/(1+cell_alpha*cell_alpha); //m_m_gradx_m=(beta-alpha)/(1+alpha^2)*u*m^(m^pm_px)
				inter_dEdt -= m_m_grady_m; //there was an error of sign here
				m_grady_m *= (cell_alpha*beta+1)/(1+cell_alpha*cell_alpha); //m_gradx_m=(1+alpha*beta)/(1+alpha^2)*u*m^pmpx
				inter_dEdt += m_grady_m; //inter_dEdt=-alpha*|gamma_LL|*m^H+(beta-alpha)/(1+alpha^2)*u*pm_px+(1+alpha*beta)/(1+alpha^2)*u*m^pmpx
}
...
void Anv_SpinT2DEvolve::CheckCache(const Oxs_SimState& cstate)
{
  ...
  if(!cache_good) {
    throw Oxs_Ext::Error(this,
       "Anv_SpinT2DEvolve::CheckCache: Invalid data cache.");
  }
}


void
Anv_SpinT2DEvolve::AdjustState
(OC_REAL8m hstep,
 OC_REAL8m mstep,
 ...
 OC_REAL8m& norm_error) const
{
  ...
  const OC_UINT4m size = old_state.mesh->Size();
  ...
  // to Anv_SpinT2DEvolve::Step() takes 2 half-steps, and it is the
  ...
  // in by the Anv_SpinT2DEvolve::NegotiateTimeStep() method.
}


void Anv_SpinT2DEvolve::UpdateTimeFields
...
void Anv_SpinT2DEvolve::NegotiateTimeStep
...
{ // This routine negotiates with driver over the proper step size.
  ...
  if(!cstate.GetDerivedData("Max dm/dt",max_dm_dt)) {
    throw Oxs_Ext::Error(this,
       "Anv_SpinT2DEvolve::NegotiateTimeStep: max_dm_dt not cached.");
  }
  ...
  if(!cstate.GetDerivedData("Timestep lower bound",
			    timestep_lower_bound)) {
    throw Oxs_Ext::Error(this,
       "Anv_SpinT2DEvolve::NegotiateTimeStep: "
...
void Anv_SpinT2DEvolve::TakeRungeKuttaStep2
(OC_REAL8m stepsize,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  OC_UINT4m i;
  const OC_UINT4m size = cstate->mesh->Size();
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
    ...
    throw Oxs_Ext::Error(this,
			 "Anv_SpinT2DEvolve::TakeRungeKuttaStep2:"
...
void Anv_SpinT2DEvolve::TakeRungeKuttaStep4
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Anv_SpinT2DEvolve::RungeKuttaFehlbergBase54
(RKF_SubType method,
 OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
...
{ // Runge-Kutta-Fehlberg routine with combined 4th and 5th
  ...
  default:
    throw Oxs_Ext::Error(this,
		 "Anv_SpinT2DEvolve::RungeKuttaFehlbergBase54:"
...
void Anv_SpinT2DEvolve::TakeRungeKuttaFehlbergStep54
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Anv_SpinT2DEvolve::TakeRungeKuttaFehlbergStep54M
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Anv_SpinT2DEvolve::TakeRungeKuttaFehlbergStep54S
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
OC_REAL8m Anv_SpinT2DEvolve::MaxDiff
(const Oxs_MeshValue<ThreeVector>& vecA,
 const Oxs_MeshValue<ThreeVector>& vecB)
{
  OC_UINT4m size = vecA.Size();
  if(vecB.Size()!=size) {
    throw Oxs_Ext::Error(this,
		 "Anv_SpinT2DEvolve::MaxDiff:"
...
void Anv_SpinT2DEvolve::AdjustStepHeadroom(OC_INT4m step_reject)
...
OC_BOOL
Anv_SpinT2DEvolve::Step(const Oxs_TimeDriver* driver,
                      Oxs_ConstKey<Oxs_SimState> current_state_key,
                      ...
                      Oxs_Key<Oxs_SimState>& next_state_key)
{
  ...
  if(next_timestep<=0.0 ||
    ...
    if(cstate.stage_number==0
      ...
      throw Oxs_Ext::Error(this,
	   "Anv_SpinT2DEvolve::Step; Programming error:"
  ...
  if(!cstate.GetDerivedData("dE/dt",current_dE_dt)) {
    throw Oxs_Ext::Error(this,"Anv_SpinT2DEvolve::Step:"
  ...
  OC_UINT4m i;
  const OC_UINT4m size = nstate.mesh->Size();
  for(i=0;i<size;++i) {
...
void Anv_SpinT2DEvolve::UpdateDerivedOutputs(const Oxs_SimState& state)
{ // This routine fills all the Anv_SpinT2DEvolve Oxs_ScalarOutput's to
...
void Anv_SpinT2DEvolve::UpdateSpinTorqueOutputs(const Oxs_SimState& state)
{
    ...
    for(i=0;i<size;i++) {
      ...
      //const OC_REAL8m cell_mgamma = -1*gamma[i]; // -1 * Landau-Lifshitz gamma
			...
			if(x>0) {
				j = i-1;
				if(Ms[j]!=0.0) {
					gradx_m = 0.5*(base -spin_[j]);
				}
			}
			if(x<n_x-1) {
				j = i+1;
				if(Ms[j]!=0.0) {
					gradx_m += 0.5*(spin_[j] - base);
				}
			}
			if(n_x>2) { // Free boundary correction
				if(x==0) {
			...
			m_m_gradx_m = m_gradx_m;
			m_m_gradx_m ^= spin_[i]; //u*(m^pm_px)^m=-u*m^(m^pm_px)
			...
			vtmp += scratch2; //vtmp = -u*pm_px+beta*u*m^pm_px 

			stt[i] = vtmp; // Note that this is zero at fixed spin sites!!!
      }
    }
