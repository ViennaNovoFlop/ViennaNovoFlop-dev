...
#include "simstate.h"
#include "Xiao_2If_STTEvolve.h"	// TW
...
OXS_EXT_REGISTER(X2IF_STTEvolve);

/* End includes */

// Constructor
X2IF_STTEvolve::X2IF_STTEvolve(
...
// 2015-09-30 TW --> added it as const in the class definition
//  hbar = 1.054571800e-34;  // Js
//  el   = 1.6021766208e-19; // As
//  mu0  = 4.0*PI*1.0e-7;    // Vs/Am
...
if(HasInitValue("J")) {
  ...
  if(badinput1 || badinput2) {
    String msg=
      String("Bad surface parameter block, inside "
             "Oxs_X2IF_STTEvolve mif Specify block for object ")
  ...
  if(method.compare("rk2")==0) {
    rkstep_ptr = &X2IF_STTEvolve::TakeRungeKuttaStep2;
  } else if(method.compare("rk4")==0) {
    rkstep_ptr = &X2IF_STTEvolve::TakeRungeKuttaStep4;
  } else if(method.compare("rkf54m")==0) {
    rkstep_ptr = &X2IF_STTEvolve::TakeRungeKuttaFehlbergStep54M;
  } else if(method.compare("rkf54s")==0) {
    rkstep_ptr = &X2IF_STTEvolve::TakeRungeKuttaFehlbergStep54S;
  } else if(method.compare("rkf54")==0) {
    rkstep_ptr = &X2IF_STTEvolve::TakeRungeKuttaFehlbergStep54;
  } else {
    throw Oxs_Ext::Error(this,"Invalid initialization detected:"
  ...
  max_dm_dt_output.Setup(this,InstanceName(),"Max dm/dt","deg/ns",0,
     &X2IF_STTEvolve::UpdateDerivedOutputs);
  ...
  average_J_output.Setup(this,InstanceName(),"Average J","A/m2",0,
     &X2IF_STTEvolve::UpdateSpinTorqueOutputs);
  //average_Interface1_Torque_output.Setup(this,InstanceName(),"Average Interface 1 Torque Strength","A/m",0,
  //   &X2IF_STTEvolve::UpdateSpinTorqueOutputs);
  //average_Interface2_Torque_output.Setup(this,InstanceName(),"Average Interface 2 Torque Strength","A/m",0,
  //   &X2IF_STTEvolve::UpdateSpinTorqueOutputs);
  spin_torque_interface1_output.Setup(this,InstanceName(),"Spin torque interface 1","A/m",0,
     &X2IF_STTEvolve::UpdateSpinTorqueOutputs);
  spin_torque_interface2_output.Setup(this,InstanceName(),"Spin torque interface 2","A/m",0,
     &X2IF_STTEvolve::UpdateSpinTorqueOutputs);
  ...
  //average_Interface1_Torque_output.Register(director,-5);
  //average_Interface2_Torque_output.Register(director,-5);
...
void X2IF_STTEvolve::UpdateMeshArrays(const Oxs_RectangularMesh* mesh)
{
  ...
  if(rmesh==NULL) {
    String msg="Import mesh to X2IF_STTEvolve::UpdateMeshArrays"
...
OC_REAL8m X2IF_STTEvolve::EvaluateJProfileScript
(OC_UINT4m stage,
...
void X2IF_STTEvolve::Calculate_dm_dt
...
//////////////////////////////////////////////////////////////////////////////////////////
			...
			//const OC_REAL8m cell_mgamma = -1.*fabs(gamma[i]); // -1 * Landau-Lifshitz gamma gamma_LL=gamma_G/(1+alpha^2)
			const OC_REAL8m cell_gamma = fabs(gamma[i]); 
			...
			scratch *= cell_alpha*cell_gamma; //scratch = -alpha*gamma_LL*m^(m^H)
...
// Energy contribution from dissipation alpha* mxmxH
      ...
      if(dm_dt_sq>0)	{
				inter_dEdt += mxH;
				inter_dEdt *= cell_alpha*cell_gamma; // inter_dEdt=-alpha*|gamma_LL|*m^H
...
// 2015-10-02 TW added dE_dt for energy calculation of interface
 ...
 for(it=links_if1.begin();it!=links_if1.end();++it) {
	...
	const OC_REAL8m cell_mgamma1 = -1*gamma[i]; 
...
/////////////////////////////////////////////////////////////////////////
 ...
 for(it=links_if2.begin();it!=links_if2.end();++it) {
	...
	const OC_REAL8m cell_mgamma2 = -1*gamma[i]; 
...
void X2IF_STTEvolve::CheckCache(const Oxs_SimState& cstate)
{
  ...
  if(!cache_good) {
    throw Oxs_Ext::Error(this,
       "X2IF_STTEvolve::CheckCache: Invalid data cache.");
  }
}


void
X2IF_STTEvolve::AdjustState
(OC_REAL8m hstep,
 OC_REAL8m mstep,
 ...
 OC_REAL8m& norm_error) const
{
  ...
  // to X2IF_STTEvolve::Step() takes 2 half-steps, and it is the
  ...
  // in by the X2IF_STTEvolve::NegotiateTimeStep() method.
}


void X2IF_STTEvolve::UpdateTimeFields
...
void X2IF_STTEvolve::NegotiateTimeStep
...
{ // This routine negotiates with driver over the proper step size.
  ...
  if(!cstate.GetDerivedData("Max dm/dt",max_dm_dt)) {
    throw Oxs_Ext::Error(this,
       "X2IF_STTEvolve::NegotiateTimeStep: max_dm_dt not cached.");
  }
  ...
  if(!cstate.GetDerivedData("Timestep lower bound",
			    timestep_lower_bound)) {
    throw Oxs_Ext::Error(this,
       "X2IF_STTEvolve::NegotiateTimeStep: "
...
void X2IF_STTEvolve::TakeRungeKuttaStep2
(OC_REAL8m stepsize,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
    ...
    throw Oxs_Ext::Error(this,
			 "X2IF_STTEvolve::TakeRungeKuttaStep2:"
...
void X2IF_STTEvolve::TakeRungeKuttaStep4
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void X2IF_STTEvolve::RungeKuttaFehlbergBase54
(RKF_SubType method,
 OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
...
{ // Runge-Kutta-Fehlberg routine with combined 4th and 5th
  ...
  default:
    throw Oxs_Ext::Error(this,
		 "X2IF_STTEvolve::RungeKuttaFehlbergBase54:"
...
void X2IF_STTEvolve::TakeRungeKuttaFehlbergStep54
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void X2IF_STTEvolve::TakeRungeKuttaFehlbergStep54M
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void X2IF_STTEvolve::TakeRungeKuttaFehlbergStep54S
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
OC_REAL8m X2IF_STTEvolve::MaxDiff
...
void X2IF_STTEvolve::AdjustStepHeadroom(OC_INT4m step_reject)
...
OC_BOOL
X2IF_STTEvolve::Step(const Oxs_TimeDriver* driver,
                      Oxs_ConstKey<Oxs_SimState> current_state_key,
                      ...
                      Oxs_Key<Oxs_SimState>& next_state_key)
{
  ...
  if(next_timestep<=0.0 ||
    ...
    if(cstate.stage_number==0
      ...
      throw Oxs_Ext::Error(this,
	   "X2IF_STTEvolve::Step; Programming error:"
  ...
  if(!cstate.GetDerivedData("dE/dt",current_dE_dt)) {
    throw Oxs_Ext::Error(this,"X2IF_STTEvolve::Step:"
...
void X2IF_STTEvolve::UpdateDerivedOutputs(const Oxs_SimState& state)
{ // This routine fills all the X2IF_STTEvolve Oxs_ScalarOutput's to
...
void X2IF_STTEvolve::UpdateSpinTorqueOutputs(const Oxs_SimState& state)
{
...
if(spin_torque_interface1_output.GetCacheRequestCount()>0) {
...
if(spin_torque_interface2_output.GetCacheRequestCount()>0 ) {
...
//  if(spin_torque_interface1_output.GetCacheRequestCount()>0) {
//     average_Interface1_Torque_output.cache.value = 1.;
//     average_Interface1_Torque_output.cache.state_id = state.Id();
//     }

//  if(spin_torque_interface2_output.GetCacheRequestCount()>0) {
//     average_Interface2_Torque_output.cache.value = 1.;
//     average_Interface2_Torque_output.cache.state_id = state.Id();
//     }
...
void X2IF_STTEvolve::FillLinkList
