...
OXS_EXT_REGISTER(Oxs_SpinSHSTTEvolve);

/* End includes */

// Constructor
Oxs_SpinSHSTTEvolve::Oxs_SpinSHSTTEvolve(
...
//----------------------------------------------------------------------------------------------------
  ...
  if(method.compare("rk2")==0) {
    rkstep_ptr = &Oxs_SpinSHSTTEvolve::TakeRungeKuttaStep2;
  } else if(method.compare("rk2heun")==0) {
    rkstep_ptr = &Oxs_SpinSHSTTEvolve::TakeRungeKuttaStep2Heun;
  } else if(method.compare("rk4")==0) {
    rkstep_ptr = &Oxs_SpinSHSTTEvolve::TakeRungeKuttaStep4;
  } else if(method.compare("rkf54m")==0) {
    rkstep_ptr = &Oxs_SpinSHSTTEvolve::TakeRungeKuttaFehlbergStep54M;
  } else if(method.compare("rkf54s")==0) {
    rkstep_ptr = &Oxs_SpinSHSTTEvolve::TakeRungeKuttaFehlbergStep54S;
  } else if(method.compare("rkf54")==0) {
    rkstep_ptr = &Oxs_SpinSHSTTEvolve::TakeRungeKuttaFehlbergStep54;
  } else {
    throw Oxs_ExtError(this,"Invalid initialization detected:"
  ...
  max_dm_dt_output.Setup(this,InstanceName(),"Max dm/dt","deg/ns",0,
     &Oxs_SpinSHSTTEvolve::UpdateDerivedOutputs);
  ...
  ave_J_output.Setup(this,InstanceName(),"average J","A/m^2",0,
     &Oxs_SpinSHSTTEvolve::UpdateSpinTorqueOutputs);
...
OC_BOOL Oxs_SpinSHSTTEvolve::Init()
{
...
void Oxs_SpinSHSTTEvolve::UpdateMeshArrays(const Oxs_Mesh* mesh)
{ 
//Debugging
//  throw Oxs_ExtError(this,"Reached start of UpdateMeshArrays()");
  ...
  if(rmesh==NULL) {
    String msg="Import mesh to Oxs_SpinSHSTTEvolve::UpdateMeshArrays"
...
OC_REAL8m Oxs_SpinSHSTTEvolve::EvaluateJProfileScript
(OC_UINT4m stage,
...
OC_REAL8m Oxs_SpinSHSTTEvolve::EvaluateICSHProfileScript
(OC_UINT4m stage,
 OC_REAL8m stage_time,
 OC_REAL8m total_time) const
{
...
OC_REAL8m Oxs_SpinSHSTTEvolve::PositiveTimestepBound(OC_REAL8m max_dm_dt)
...
void Oxs_SpinSHSTTEvolve::Calculate_dm_dt
...
//      OC_REAL8m   coef1 = 1.;
// Slonczewski function g(m.p) for metal barrier
      OC_REAL8m mdotp  = (m*p);
      mdotp += 3.;
      mdotp *= (1.+polarization[i]);
      mdotp *= (1.+polarization[i]);
      mdotp *= (1.+polarization[i]);
      mdotp *= pow(polarization[i],1.5);
      mdotp /= 4.;
      mdotp -= 4.;
      OC_REAL8m g_func = 1.;
      g_func /= mdotp;
...
void Oxs_SpinSHSTTEvolve::CheckCache(const Oxs_SimState& cstate)
{
  ...
  if(!cache_good) {
    throw Oxs_ExtError(this,
       "Oxs_SpinSHSTTEvolve::CheckCache: Invalid data cache.");
  }
}


void
Oxs_SpinSHSTTEvolve::AdjustState
(OC_REAL8m hstep,
 OC_REAL8m mstep,
 ...
 OC_REAL8m& norm_error) const
{
  ...
  // to Oxs_SpinSHSTTEvolve::Step() takes 2 half-steps, and it is the
  ...
  // in by the Oxs_SpinSHSTTEvolve::NegotiateTimeStep() method.
}


void Oxs_SpinSHSTTEvolve::UpdateTimeFields
...
void Oxs_SpinSHSTTEvolve::NegotiateTimeStep
...
{ // This routine negotiates with driver over the proper step size.
  ...
  if(!cstate.GetDerivedData("Max dm/dt",max_dm_dt)) {
    throw Oxs_ExtError(this,
       "Oxs_SpinSHSTTEvolve::NegotiateTimeStep: max_dm_dt not cached.");
  }
  ...
  if(!cstate.GetDerivedData("Timestep lower bound",
                            timestep_lower_bound)) {
    throw Oxs_ExtError(this,
       "Oxs_SpinSHSTTEvolve::NegotiateTimeStep: "
...
OC_BOOL
Oxs_SpinSHSTTEvolve::CheckError
...
void Oxs_SpinSHSTTEvolve::TakeRungeKuttaStep2
(OC_REAL8m stepsize,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
    ...
    throw Oxs_ExtError(this,
                         "Oxs_SpinSHSTTEvolve::TakeRungeKuttaStep2:"
...
void Oxs_SpinSHSTTEvolve::TakeRungeKuttaStep2Heun
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
     ...
     !endstate->AddDerivedData("dE/dt",dE_dt)) {
    throw Oxs_ExtError(this,
                         "Oxs_SpinSHSTTEvolve::TakeRungeKuttaStep2Heun:"
                         " Programming error; data cache already set.");
  }
...
void Oxs_SpinSHSTTEvolve::TakeRungeKuttaStep4
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_SpinSHSTTEvolve::RungeKuttaFehlbergBase54
(RKF_SubType method,
 OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
...
{ // Runge-Kutta-Fehlberg routine with combined 4th and 5th
  ...
  default:
    throw Oxs_ExtError(this,
                 "Oxs_SpinSHSTTEvolve::RungeKuttaFehlbergBase54:"
...
void Oxs_SpinSHSTTEvolve::TakeRungeKuttaFehlbergStep54
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_SpinSHSTTEvolve::TakeRungeKuttaFehlbergStep54M
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_SpinSHSTTEvolve::TakeRungeKuttaFehlbergStep54S
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
OC_REAL8m Oxs_SpinSHSTTEvolve::MaxDiff
...
void Oxs_SpinSHSTTEvolve::AdjustStepHeadroom(OC_INT4m step_reject)
...
OC_BOOL
Oxs_SpinSHSTTEvolve::Step(const Oxs_TimeDriver* driver,
                      Oxs_ConstKey<Oxs_SimState> current_state_key,
                      ...
                      Oxs_Key<Oxs_SimState>& next_state_key)
{
  ...
  if(next_timestep<=0.0 ||
    ...
    if(cstate.stage_number==0
      ...
      throw Oxs_ExtError(this,
           "Oxs_SpinSHSTTEvolve::Step; Programming error:"
  ...
  // -mjd, 21-July-2004
  {
    ...
    if(!cstate.GetDerivedData("dE/dt",current_dE_dt)) {
      throw Oxs_ExtError(this,"Oxs_SpinSHSTTEvolve::Step:"
...
void Oxs_SpinSHSTTEvolve::UpdateDerivedOutputs(const Oxs_SimState& state)
{ // This routine fills all the Oxs_SpinSHSTTEvolve Oxs_ScalarOutput's to
...
void Oxs_SpinSHSTTEvolve::UpdateSpinTorqueOutputs(const Oxs_SimState& state)
{
