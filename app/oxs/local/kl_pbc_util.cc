...
inline OC_REALWIDE CalculateSDA00_dip(OC_REALWIDE x,OC_REALWIDE y,OC_REALWIDE z)
{ // This is Nxx term (or N00) from eq. (15) without the mul-factor
  return (2.*x*x-y*y-z*z) / (3.*sqrt( pow5(x*x+y*y+z*z) ));
}
inline OC_REALWIDE CalculateSDA11_dip(OC_REALWIDE x, OC_REALWIDE y, OC_REALWIDE z)
{ return CalculateSDA00_dip(y,x,z); }
inline OC_REALWIDE CalculateSDA22_dip(OC_REALWIDE x, OC_REALWIDE y, OC_REALWIDE z)
{ return CalculateSDA00_dip(z,y,x); }
//
inline OC_REALWIDE CalculateSDA01_dip(OC_REALWIDE x,OC_REALWIDE y,OC_REALWIDE z)
{ // This is Nxy term (or N01) from eq. (15) without the mul-factor
  return x*y / sqrt( pow5(x*x+y*y+z*z) );
}
inline OC_REALWIDE CalculateSDA02_dip(OC_REALWIDE x, OC_REALWIDE y, OC_REALWIDE z)
{ return CalculateSDA01_dip(x,z,y); }
inline OC_REALWIDE CalculateSDA12_dip(OC_REALWIDE x, OC_REALWIDE y, OC_REALWIDE z)
...
OC_REALWIDE CalculateSDA00_tails(OC_REALWIDE x,OC_REALWIDE y,OC_REALWIDE z,
			OC_REALWIDE zPeriod, OC_UINT4m n_max)
{ 
...
OC_REALWIDE CalculateSDA01_tails(OC_REALWIDE x,OC_REALWIDE y,OC_REALWIDE z,
			OC_REALWIDE zPeriod, OC_UINT4m n_max)
{ 
  const OC_REALWIDE nmin05=static_cast<OC_REALWIDE>(n_max)-0.5;	
...
OC_REALWIDE CalculateSDA02_tails(OC_REALWIDE x,OC_REALWIDE y,OC_REALWIDE z,
			OC_REALWIDE zPeriod, OC_UINT4m n_max)
{ 
  const OC_REALWIDE nmin05=static_cast<OC_REALWIDE>(n_max)-0.5;	
  return ( \
...
OC_REALWIDE CalculateSDA22_tails(OC_REALWIDE x,OC_REALWIDE y,OC_REALWIDE z,
			OC_REALWIDE zPeriod, OC_UINT4m n_max)
{ 
  const OC_REALWIDE nmin05=static_cast<OC_REALWIDE>(n_max)-0.5;	
  return ( \
...
inline OC_REALWIDE CalculateSDA11_tails(OC_REALWIDE x,OC_REALWIDE y,OC_REALWIDE z,
			OC_REALWIDE zPeriod, OC_UINT4m n_max)
{ return CalculateSDA00_tails(y,x,z,zPeriod,n_max); }
        
inline OC_REALWIDE CalculateSDA12_tails(OC_REALWIDE x,OC_REALWIDE y,OC_REALWIDE z,
			OC_REALWIDE zPeriod, OC_UINT4m n_max)
{ return CalculateSDA02_tails(y,x,z,zPeriod,n_max); }

// helper functions
inline OC_REAL4m powI4(const OC_UINT4m n) {
  return static_cast<OC_REAL4m>(n)*static_cast<OC_REAL4m>(n)
        *static_cast<OC_REAL4m>(n)*static_cast<OC_REAL4m>(n);
}
inline OC_REAL4m powI6(const OC_UINT4m n) {
...
//   |scale|.
OC_REAL4m CalculateSDA00_cont_err(OC_REALWIDE /*x*/,OC_REALWIDE /*y*/,OC_REALWIDE /*z*/,
			        OC_REALWIDE zPeriod, OC_UINT4m n) { 
  return 1. / (4.*powR3(zPeriod)*powI4(n));
}			        
inline OC_REAL4m CalculateSDA11_cont_err(OC_REALWIDE x,OC_REALWIDE y,OC_REALWIDE z,
			        OC_REALWIDE zPeriod, OC_UINT4m n) { 
  return CalculateSDA00_cont_err(y, x, z, zPeriod, n);
}			        
OC_REAL4m CalculateSDA22_cont_err(OC_REALWIDE /*x*/,OC_REALWIDE /*y*/,OC_REALWIDE /*z*/,
			        OC_REALWIDE zPeriod, OC_UINT4m n) { 
  return 1 / (2.*powR3(zPeriod)*powI4(n));
}			        
OC_REAL4m CalculateSDA01_cont_err(OC_REALWIDE x,OC_REALWIDE y,OC_REALWIDE /*z*/,
			        OC_REALWIDE zPeriod, OC_UINT4m n) { 
  return 5.*static_cast<OC_REAL4m>(x)*static_cast<OC_REAL4m>(y) \
           / (4.*powR5(zPeriod)*powI6(n));
}			        
OC_REAL4m CalculateSDA02_cont_err(OC_REALWIDE x,OC_REALWIDE /*y*/,OC_REALWIDE z,
			        OC_REALWIDE zPeriod, OC_UINT4m n) { 
  return 5.*static_cast<OC_REAL4m>(x)*static_cast<OC_REAL4m>(z) \
           / (powR5(zPeriod)*powI6(n));
}			        
inline OC_REAL4m CalculateSDA12_cont_err(OC_REALWIDE x,OC_REALWIDE y,OC_REALWIDE z,
			        OC_REALWIDE zPeriod, OC_UINT4m n) { 
...
OC_REAL4m CalculateSDA_dip_err_accumulated(OC_REALWIDE zPeriod, OC_UINT4m n) { 
  return 4.*PI / (3.*powR7(zPeriod)*powI6(n));
}			        

// Helper function
OC_UINT4m NextPowerOfTwo(OC_UINT4m n)
...
DemagPBC::DemagPBC( 
      ...
      const OC_UINT4m 	max_no_sum_elements_param,
      const OC_UINT4m 	no_of_sum_elements_param,
	...
	include_inf_tails		(include_inf_tails_param),
        ...
        tensor_buffer(NULL)
{ 
  // highest possible OC_UINT4m value
  OC_UINT4m MAX_UINT = 0;
  MAX_UINT--;
  ...
  if(tensor_file_name_param!="") {
    if(tensor_file_name_param.substr( tensor_file_name_param.length()-1,
      ...
      Oc_Snprintf(buf,sizeof(buf),"t-%d-%d-%d-%d-",xdim,ydim,zdim,
      				  static_cast<OC_UINT4m>(OC_ROUND(zPeriod/dz)));
...
OC_REAL4m DemagPBC::CalculateSDAerrIMG_2(const OC_REALWIDE r2, 
...
OC_REAL4m DemagPBC::CalculateSDAerrDIP_2(const OC_REALWIDE r2, 
                                      const T_tens_el teXX) const
{ 
  if(compute_stats) {
    if(teXX==n00 || teXX==n11 || teXX==n22)
...
{
  String tmp = str;
  OC_UINT4m i = tmp.find(replace_what);
  while(i != (OC_UINT4m)String::npos) { // casting to avoid compiler warnings
...
// As for now it is not implemented.
{
  ...
  try { // we must ensure Delete of file_mesh 
    ...
    // mesh size
    if(static_cast<OC_UINT4m>(file_mesh->GetSize())!=mesh->Size()) {
      char buf[4096];
      Oc_Snprintf(buf,sizeof(buf),
    		"\nDifferent mesh sizes:\n"
    		"%d != %d\n"
    		,static_cast<OC_UINT4m>(file_mesh->GetSize())
        ,mesh->Size());
      throw Oxs_Ext::Error(buf);
    }
    if(tensor_buffer.Size() != mesh->Size())
    ...
    // you can use it...
    /* const OC_UINT4m f_noX = \
        static_cast<OC_UINT4m>(OC_ROUND(file_range.GetWidth() /file_dim.x));
    const OC_UINT4m f_noY = \
        static_cast<OC_UINT4m>(OC_ROUND(file_range.GetHeight()/file_dim.y));
    const OC_UINT4m f_noZ = \
        static_cast<OC_UINT4m>(OC_ROUND(file_range.GetDepth() /file_dim.z));
    const OC_UINT4m noX = \
        static_cast<OC_UINT4m>(OC_ROUND((bbox.GetMaxX()-bbox.GetMinX())/edgeX));
    const OC_UINT4m noY = \
        static_cast<OC_UINT4m>(OC_ROUND((bbox.GetMaxY()-bbox.GetMinY())/edgeY));
    const OC_UINT4m noZ = \
        static_cast<OC_UINT4m>(OC_ROUND((bbox.GetMaxZ()-bbox.GetMinZ())/edgeZ));
    ...
    // Fill the buffer
    OC_UINT4m size = tensor_buffer.Size();
...
OC_REAL8m DemagPBC::GetTensorElementFromBuffer(
    const OC_UINT4m el_idx, const T_tens_el teXX) const 
{
  switch(exec_order) {
    case _SimpleDemag:
      switch(teXX) {
...
void DemagPBC::PutTensorElementToBuffer(
    const OC_REAL8m elem, const OC_UINT4m el_idx, const T_tens_el teXX) const
{    
  switch(exec_order) {
    case _SimpleDemag:
      switch(teXX) { 
...
OC_REALWIDE DemagPBC::CalculateSDA_PBC_really( \
    const OC_UINT4m i, const OC_UINT4m j, const OC_UINT4m k, \
    const T_tens_el teXX) const
{ // conceptually const
  ...
  OC_REALWIDE \
    (*CalculateSDAXX)(OC_REALWIDE,OC_REALWIDE,OC_REALWIDE,OC_REALWIDE,OC_REALWIDE,OC_REALWIDE) = NULL,
    (*CalculateSDAXX_dip)(OC_REALWIDE,OC_REALWIDE,OC_REALWIDE)                        = NULL,
    (*CalculateSDAXX_tails)(OC_REALWIDE,OC_REALWIDE,OC_REALWIDE,OC_REALWIDE,OC_UINT4m)      = NULL;
  OC_REAL4m \
    (*CalculateSDAXX_cont_err)(OC_REALWIDE,OC_REALWIDE,OC_REALWIDE,OC_REALWIDE,OC_UINT4m)   = NULL;
  switch(teXX) {
    case n00: 
  ...
  OC_UINT4m MAX_UINT = 0; // for range overflow checking
  MAX_UINT--;
  OC_UINT4m n=0; // repetinion counter
  do 
  {
    n++; // we start with n==1
...
//                           && n<max_no_sum_elements) || // safety stopper, for instance when constantly coef==0
  ...
  // "Infinite tails"
  const OC_REALWIDE infinity_tails = (*CalculateSDAXX_tails)(x,y,z,zPeriod,n+1);
...
OC_REALWIDE DemagPBC::CalculateSDA(const T_tens_el teXX,
                  const OC_UINT4m i, const OC_UINT4m j, const OC_UINT4m k) const
{ // conceptually const
  OC_REALWIDE coef;
  ...
  if(tensor_rw==NOTHING || tensor_rw==WRITE) {
    ...
    if(show_progress==1) { // progress
      ...
      OC_UINT4m actual_cnt = (cell_cnt * progress_steps) / (totalsize * 6);
