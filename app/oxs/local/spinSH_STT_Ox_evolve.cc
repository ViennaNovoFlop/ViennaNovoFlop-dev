...
#include "simstate.h"
#include "spinSH_STT_Ox_evolve.h"
...
OXS_EXT_REGISTER(Oxs_Spin_SH_STT_Ox_Evolve);

/* End includes */

// Constructor
Oxs_Spin_SH_STT_Ox_Evolve::Oxs_Spin_SH_STT_Ox_Evolve(
...
//    throw Oxs_ExtError(this,"Test flag #1\n");
//FILE * f=fopen("/home/windbach/oommf/oommf_parallel/debug.txt","w");
//fprintf(f,"Flag #1\n");
  ...
  if(HasInitValue("P")) {
    OXS_GET_INIT_EXT_OBJECT("P",Oxs_ScalarField,P_init);
      ...
      // object with value 0.6
      // 10-Jan-2013, TW
      Oxs_Ext* foo=MakeNew("Oxs_UniformScalarField",director,"value 0.6");
      P_init.SetAsOwner(dynamic_cast<Oxs_ScalarField*>(foo));
    }
  

  if(HasInitValue("g")) {
    OXS_GET_INIT_EXT_OBJECT("g",Oxs_ScalarField,g_init);
  } else {
      // Default setting: create a uniform scalar field
      // object with value 2.0
      // 10-Jan-2013, TW
      Oxs_Ext* foo=MakeNew("Oxs_UniformScalarField",director,"value 2.0");
      g_init.SetAsOwner(dynamic_cast<Oxs_ScalarField*>(foo));
    }
...
//----------------------------------------------------------------------------------------------------
//Section adding for reading SH related parameters----------------------------------------------------
//----------------------------------------------------------------------------------------------------
    OXS_GET_INIT_EXT_OBJECT("ico",Oxs_VectorField,ico_init);
//  if(HasInitValue("ico")) {
//    OXS_GET_INIT_EXT_OBJECT("ico",Oxs_VectorField,ico_init);
//  } else {
//      Oxs_Ext* foo=MakeNew("Oxs_UniformVectorField",director,"vector {1.0 0.0 0.0}");
//      ico_init.SetAsOwner(dynamic_cast<Oxs_VectorField*>(foo));
//    }

    OXS_GET_INIT_EXT_OBJECT("iso",Oxs_VectorField,iso_init);
//  if(HasInitValue("iso")) {
//    OXS_GET_INIT_EXT_OBJECT("iso",Oxs_VectorField,ico_init);
//  } else {
//      Oxs_Ext* foo=MakeNew("Oxs_UniformVectorField",director,"vector {1.0 0.0 0.0}");
//      iso_init.SetAsOwner(dynamic_cast<Oxs_VectorField*>(foo));
//    }
// making it required for debugging
  OXS_GET_INIT_EXT_OBJECT("thetaSH",Oxs_ScalarField,thetaSH_init);
//  if(HasInitValue("thetaSH")) {
//    OXS_GET_INIT_EXT_OBJECT("thetaSH",Oxs_ScalarField,thetaSH_init);
//  } else {
//      Oxs_Ext* foo=MakeNew("Oxs_UniformScalarField",director,"value 1.0");
//      thetaSH_init.SetAsOwner(dynamic_cast<Oxs_ScalarField*>(foo));
//    }

//  if(HasInitValue("tSH")) {
    OXS_GET_INIT_EXT_OBJECT("tSH",Oxs_ScalarField,tSH_init);
//  } else {
//      Oxs_Ext* foo=MakeNew("Oxs_UniformScalarField",director,"value 2.0e-09");
//      tSH_init.SetAsOwner(dynamic_cast<Oxs_ScalarField*>(foo));
//    }

//  if(HasInitValue("wSH")) {
    OXS_GET_INIT_EXT_OBJECT("wSH",Oxs_ScalarField,wSH_init);
//  } else {
//      Oxs_Ext* foo=MakeNew("Oxs_UniformScalarField",director,"value 1.0e-09");
//      wSH_init.SetAsOwner(dynamic_cast<Oxs_ScalarField*>(foo));
//    }

//  if(HasInitValue("lambdaSF")) {
    OXS_GET_INIT_EXT_OBJECT("lambdaSF",Oxs_ScalarField,lambdaSF_init);
//  } else {
//      Oxs_Ext* foo=MakeNew("Oxs_UniformScalarField",director,"value 1.0e-09");
//      lambdaSF_init.SetAsOwner(dynamic_cast<Oxs_ScalarField*>(foo));
//    }

//  if(HasInitValue("beta")) {
    OXS_GET_INIT_EXT_OBJECT("beta",Oxs_ScalarField,beta_init);
//  } else {
//    beta_init.SetAsOwner(dynamic_cast<Oxs_ScalarField *>
//                          (MakeNew("Oxs_UniformScalarField",director,
//                                   "value 0.0")));// 0. --> no field like torque component
//  }
 
  if(HasInitValue("I_CSH")) {
    OXS_GET_INIT_EXT_OBJECT("I_CSH",Oxs_ScalarField,Icsh_init);    // Required
  } else {
    Icsh_init.SetAsOwner(dynamic_cast<Oxs_ScalarField *>
                     (MakeNew("Oxs_UniformScalarField",director,"value 0.0")));// torque is set to zero if not specified 
  }

  // Stage-varying multiplier for I_CSH  Note sign convention, as
  // outlined in previous stanza.
  if(HasInitValue("ICSH_profile")) {
    has_ICSH_profile = 1;
    String cmdoptreq = GetStringInitValue("ICSH_profile_args",
                                          "stage stage_time total_time");
    ICSH_profile_opts.push_back(Oxs_TclCommandLineOption("stage",1));
    ICSH_profile_opts.push_back(Oxs_TclCommandLineOption("stage_time",1));
    ICSH_profile_opts.push_back(Oxs_TclCommandLineOption("total_time",1));
    ICSH_profile_cmd.SetBaseCommand(InstanceName(),
                   director->GetMifInterp(),
                   GetStringInitValue("ICSH_profile"),
                   Oxs_ParseTclCommandLineRequest(InstanceName(),
                                                  ICSH_profile_opts,
                                                  cmdoptreq));
   }
//----------------------------------------------------------------------------------------------------
  ...
  if(method.compare("rk2")==0) {
    rkstep_ptr = &Oxs_Spin_SH_STT_Ox_Evolve::TakeRungeKuttaStep2;
  } else if(method.compare("rk2heun")==0) {
    rkstep_ptr = &Oxs_Spin_SH_STT_Ox_Evolve::TakeRungeKuttaStep2Heun;
  } else if(method.compare("rk4")==0) {
    rkstep_ptr = &Oxs_Spin_SH_STT_Ox_Evolve::TakeRungeKuttaStep4;
  } else if(method.compare("rkf54m")==0) {
    rkstep_ptr = &Oxs_Spin_SH_STT_Ox_Evolve::TakeRungeKuttaFehlbergStep54M;
  } else if(method.compare("rkf54s")==0) {
    rkstep_ptr = &Oxs_Spin_SH_STT_Ox_Evolve::TakeRungeKuttaFehlbergStep54S;
  } else if(method.compare("rkf54")==0) {
    rkstep_ptr = &Oxs_Spin_SH_STT_Ox_Evolve::TakeRungeKuttaFehlbergStep54;
  } else {
    throw Oxs_ExtError(this,"Invalid initialization detected:"
  ...
  max_dm_dt_output.Setup(this,InstanceName(),"Max dm/dt","deg/ns",0,
     &Oxs_Spin_SH_STT_Ox_Evolve::UpdateDerivedOutputs);
  ...
  ave_J_output.Setup(this,InstanceName(),"average J","A/m^2",0,
     &Oxs_Spin_SH_STT_Ox_Evolve::UpdateSpinTorqueOutputs);
  ave_ICSH_output.Setup(this,InstanceName(),"average I_CSH","A",0,
     &Oxs_Spin_SH_STT_Ox_Evolve::UpdateSpinTorqueOutputs);
  ...
  ave_ICSH_output.Register(director,-5);
...
//Debugging
//    throw Oxs_ExtError(this,"Reached end of constructor");
}

OC_BOOL Oxs_Spin_SH_STT_Ox_Evolve::Init()
{
  ...
  // Free memory used by LLG damping parameter alpha, gamma etc.
  alpha.Release();
  gamma.Release();
  g.Release();
  beta.Release();
...
//Debugging
//  throw Oxs_ExtError(this,"Reached end of Init()");
...
void Oxs_Spin_SH_STT_Ox_Evolve::UpdateMeshArrays(const Oxs_Mesh* mesh)
{ 
//Debugging
//  throw Oxs_ExtError(this,"Reached start of UpdateMeshArrays()");
  mesh_id = 0; // Mark update in progress
  const Oxs_RectangularMesh* rmesh
    = dynamic_cast<const Oxs_RectangularMesh*>(mesh);
  if(rmesh==NULL) {
    String msg="Import mesh to Oxs_Spin_SH_STT_Ox_Evolve::UpdateMeshArrays"
  ...
  const OC_REAL8m bohr_magneton = 9.27400968e-24;// J/T
  OC_REAL8m coeff2b  = fabs(bohr_magneton/(e_charge));
  if(cell_thickness>0.) {coeff2b /= cell_thickness;} 
    else { coeff2b   = 0.0;}; // Safety
//---------------------------------------------------------------
//Adjustments for SH-torque-model--------------------------------
//---------------------------------------------------------------

Icsh_init->FillMeshValue(mesh,A2); // Uses A2 as temp space for Spin Hall charge current
thetaSH_init->FillMeshValue(mesh,thetaSH); // spin Hall angle of employed material
beta_init->FillMeshValue(mesh,beta);
tSH_init->FillMeshValue(mesh,tSH);
wSH_init->FillMeshValue(mesh,wSH);
lambdaSF_init->FillMeshValue(mesh,lambdaSF);
...
//---------------------------------------------------------------
  ...
  for(i=0;i<size;i++) {
   ...
   // A[i] holds J(i)
   // A2[i] holds ICSH(i)
   /// B[i] = bohr_magneton*j[i]/(e_charge*cell_thickness)
    B[i]     = coeff2b*A[i];
    B2[i]    = coeff2b*A2[i];
    B2[i]   *= thetaSH[i];
    B2[i]   /= tSH[i];
    B2[i]   /= wSH[i];
    B2[i]   *= 1.-1./cosh(tSH[i]/lambdaSF[i]);
  }
  if(size>0) {
     ave_J    /= size;
     ave_ICSH /= size;
  ...
  P_init->FillMeshValue(mesh,polarization);
  ...
  for(i=0;i<size;i++) mp[i].MakeUnit(); // These are supposed
  /// to be unit vectors!

  ico_init->FillMeshValue(mesh,ico);
  for(i=0;i<size;i++) ico[i].MakeUnit(); // These are supposed
  // to be unit vectors!

  iso_init->FillMeshValue(mesh,iso);
  for(i=0;i<size;i++) iso[i].MakeUnit(); // These are supposed
  // to be unit vectors!

  g_init->FillMeshValue(mesh,g);
  alpha_init->FillMeshValue(mesh,alpha);
  gamma_init->FillMeshValue(mesh,gamma);

// Convert to LL form and makes sure B[i] is divided by GS_G 15.1.2013
// Division by gamma is necssary due to the use of the formulation with mu_B prefactor!!!
  if(gamma_style == GS_G) {
    for(i=0;i<size;++i) {
     ...
     for(i=0;i<size;++i) {
      ...
      OC_REAL8m cell_alpha = alpha[i];
      B[i] /= gamma[i];
      B[i] /= (1.+cell_alpha*cell_alpha);      
      B2[i] /= gamma[i];
      B2[i] /= (1.+cell_alpha*cell_alpha);      
     }
  }
...
//----------------------------------------------------
  ...
  mesh_id = mesh->Id();
//throw Oxs_ExtError("Update mesh end reached");

}


OC_REAL8m Oxs_Spin_SH_STT_Ox_Evolve::EvaluateJProfileScript
(OC_UINT4m stage,
...
OC_REAL8m Oxs_Spin_SH_STT_Ox_Evolve::EvaluateICSHProfileScript
(OC_UINT4m stage,
 OC_REAL8m stage_time,
 OC_REAL8m total_time) const
{
  if(!has_ICSH_profile) return 1.0;

  int index;
  if((index = ICSH_profile_opts[0].position)>=0) { // stage
    ICSH_profile_cmd.SetCommandArg(index,stage);
  }
  if((index = ICSH_profile_opts[1].position)>=0) { // stage_time
    ICSH_profile_cmd.SetCommandArg(index,stage_time);
  }
  if((index = ICSH_profile_opts[2].position)>=0) { // total_time
    ICSH_profile_cmd.SetCommandArg(index,total_time);
  }

  ICSH_profile_cmd.SaveInterpResult();
  ICSH_profile_cmd.Eval();
  if(ICSH_profile_cmd.GetResultListSize()!=1) {
    String msg
      = String("Return script value is not a single scalar: ")
      + ICSH_profile_cmd.GetWholeResult();
    ICSH_profile_cmd.RestoreInterpResult();
    throw Oxs_ExtError(this,msg.c_str());
  }
  OC_REAL8m result;
  ICSH_profile_cmd.GetResultListItem(0,result);
  ICSH_profile_cmd.RestoreInterpResult();

//Debugging
//  throw Oxs_ExtError(this,"Reached end of UpdateMeshArrays()");
  return result;
}

OC_REAL8m Oxs_Spin_SH_STT_Ox_Evolve::PositiveTimestepBound(OC_REAL8m max_dm_dt)
...
void Oxs_Spin_SH_STT_Ox_Evolve::Calculate_dm_dt
...
//Debugging
//  throw Oxs_ExtError(this,"Reached start of Calculate_dm_dt");
...
//Debugging
//  throw Oxs_ExtError(this,"Passed nulling fixed dm_dt for spins");
...
//Debugging
//  throw Oxs_ExtError(this,"Passed call of UpdateMeshArrays within Calculate_dm_dt");
  ...
  const OC_REAL8m ICSHmult = EvaluateICSHProfileScript(state_.stage_number,
                          state_.stage_elapsed_time,
                          state_.stage_start_time+state_.stage_elapsed_time);

  if(ave_ICSH_output.GetCacheRequestCount()>0) {
    ave_ICSH_output.cache.value = ave_ICSH * ICSHmult;
    ave_ICSH_output.cache.state_id=state_.Id();
  }
  ...
  for(i=0;i<size;i++) {
    if(Ms_[i]==0) {
      ...
      //                   gamma[i] = fabs(gamma[i]); // Landau-Lifshitz gamma - ensures positive sign
      //const OC_REAL8m cell_mgamma = -1.*gamma[i];    // -1 * Landau-Lifshitz gamma
      const OC_REAL8m cell_gamma    = gamma[i];    // Landau-Lifshitz gamma
      ThreeVector p     = mp[i];
      ThreeVector m     = spin_[i];
      ThreeVector mxp   = m; mxp ^= p;
      ThreeVector mxmxp = m; mxmxp ^= mxp;
      ThreeVector mxH   = dm_dt_[i];
      ThreeVector mxmxH = m; mxmxH ^= mxH;
// for debugging purposes
//      OC_REAL8m   coef1 = 1.;
// Slonczewski function g(m.p) for oxide barrier
      OC_REAL8m mdotp  = (m*p);
      mdotp *= polarization[i];
      mdotp *= polarization[i];
      mdotp += 1.;
      mdotp *= 2.;
      OC_REAL8m g_func = polarization[i];
      g_func /= mdotp;
// multiplying this term with corresponding pre-factor
      g_func  *= B[i];
      g_func  *= Jmult;
      g_func  *= Ms_inverse_[i];
      g_func  *= g[i];

// starting to assemble the complete term -> g_func*[alpha*mxp-mxmxp]
      ThreeVector scratchA = mxp;
      scratchA *= cell_alpha;
      scratchA -= mxmxp;
      scratchA *= -1.*g_func;
/////////////////////////////////////////////////////////////////////

// now the damping term alpha* mx(mxh)
      ThreeVector scratchB = mxmxH;
      scratchB *= cell_alpha;
// and the precession term mxh
      scratchB += mxH;
// combining precession, damping and STT
      scratchA += scratchB;
// Spin-Hall section---------------------------------------------------------------------------------------
      const OC_REAL8m   cell_beta   = beta[i];
      const ThreeVector cell_ico    = ico[i];
      const ThreeVector cell_iso    = iso[i];
      // calculates the electrons polarization direction from the charge current flow direction (ico)
      // and the spin current flow direction (iso) sigma = ico x iso
      ThreeVector s        = cell_ico; s ^= cell_iso; 
      s.MakeUnit();//safety
//                       m = spin_[i];//refresh
      ThreeVector mxs      = m; mxs ^= s;
      ThreeVector mxmxs    = m; mxmxs ^= mxs;
      OC_REAL8m   g2_func  = B2[i];
                  g2_func *= Ms_inverse_[i];
                  g2_func *= g[i];
                  g2_func *= ICSHmult;
//---------------------------------------------------------------------------------------------------------
// assembling SH-torque term -> g2_func*(beta*mxs-mxmxs)
      scratchB  = mxs;
      scratchB *= cell_beta;
      scratchB -= mxmxs;
      scratchB *= -1.*g2_func;

// putting everything together to obtain the complete equation
      scratchA += scratchB;
      scratchA *= cell_gamma;

      dm_dt_[i] = scratchA;
// For debug: Check if precession and damping works as expected
      //dm_dt_[i] = gamma[i] * ( mxH + cell_alpha * mxmxH);
...
// assembling term for energy contribution of the SH-torque
     s *= -1.*cell_beta;
     s += mxs;
     s *= g2_func;

// adding term to complete the expression
     p += s;

// Energy change
      dE_dt_sum += (p*mxH)*Ms_[i]*mesh->Volume(i)*cell_gamma;
       //dE_dt_sum += (cell_alpha*(mxH*mxH))*Ms_[i]*mesh->Volume(i)*gamma[i];
...
void Oxs_Spin_SH_STT_Ox_Evolve::CheckCache(const Oxs_SimState& cstate)
{
  ...
  if(!cache_good) {
    throw Oxs_ExtError(this,
       "Oxs_Spin_SH_STT_Ox_Evolve::CheckCache: Invalid data cache.");
  }
}


void
Oxs_Spin_SH_STT_Ox_Evolve::AdjustState
(OC_REAL8m hstep,
 OC_REAL8m mstep,
 ...
 OC_REAL8m& norm_error) const
{
  ...
  // to Oxs_Spin_SH_STT_Ox_Evolve::Step() takes 2 half-steps, and it is the
  ...
  // in by the Oxs_Spin_SH_STT_Ox_Evolve::NegotiateTimeStep() method.
}


void Oxs_Spin_SH_STT_Ox_Evolve::UpdateTimeFields
...
void Oxs_Spin_SH_STT_Ox_Evolve::NegotiateTimeStep
...
{ // This routine negotiates with driver over the proper step size.
  ...
  if(!cstate.GetDerivedData("Max dm/dt",max_dm_dt)) {
    throw Oxs_ExtError(this,
       "Oxs_Spin_SH_STT_Ox_Evolve::NegotiateTimeStep: max_dm_dt not cached.");
  }
  ...
  if(!cstate.GetDerivedData("Timestep lower bound",
                            timestep_lower_bound)) {
    throw Oxs_ExtError(this,
       "Oxs_Spin_SH_STT_Ox_Evolve::NegotiateTimeStep: "
...
OC_BOOL
Oxs_Spin_SH_STT_Ox_Evolve::CheckError
...
void Oxs_Spin_SH_STT_Ox_Evolve::TakeRungeKuttaStep2
(OC_REAL8m stepsize,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
    ...
    throw Oxs_ExtError(this,
                         "Oxs_Spin_SH_STT_Ox_Evolve::TakeRungeKuttaStep2:"
...
void Oxs_Spin_SH_STT_Ox_Evolve::TakeRungeKuttaStep2Heun
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
     ...
     !endstate->AddDerivedData("dE/dt",dE_dt)) {
    throw Oxs_ExtError(this,
                         "Oxs_Spin_SH_STT_Ox_Evolve::TakeRungeKuttaStep2Heun:"
                         " Programming error; data cache already set.");
  }
...
void Oxs_Spin_SH_STT_Ox_Evolve::TakeRungeKuttaStep4
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_Spin_SH_STT_Ox_Evolve::RungeKuttaFehlbergBase54
(RKF_SubType method,
 OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
...
{ // Runge-Kutta-Fehlberg routine with combined 4th and 5th
  ...
  default:
    throw Oxs_ExtError(this,
                 "Oxs_Spin_SH_STT_Ox_Evolve::RungeKuttaFehlbergBase54:"
...
void Oxs_Spin_SH_STT_Ox_Evolve::TakeRungeKuttaFehlbergStep54
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_Spin_SH_STT_Ox_Evolve::TakeRungeKuttaFehlbergStep54M
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_Spin_SH_STT_Ox_Evolve::TakeRungeKuttaFehlbergStep54S
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
OC_REAL8m Oxs_Spin_SH_STT_Ox_Evolve::MaxDiff
 ...
 const Oxs_MeshValue<ThreeVector>& vecB)
{
  ...
  if(vecB.Size()!=size) {
    throw Oxs_ExtError(this,
                 "Oxs_Spin_SH_STT_Ox_Evolve::MaxDiff:"
...
void Oxs_Spin_SH_STT_Ox_Evolve::AdjustStepHeadroom(OC_INT4m step_reject)
...
OC_BOOL
Oxs_Spin_SH_STT_Ox_Evolve::Step(const Oxs_TimeDriver* driver,
                      Oxs_ConstKey<Oxs_SimState> current_state_key,
                      ...
                      Oxs_Key<Oxs_SimState>& next_state_key)
{
  ...
  if(next_timestep<=0.0 ||
    ...
    if(cstate.stage_number==0
      ...
      throw Oxs_ExtError(this,
           "Oxs_Spin_SH_STT_Ox_Evolve::Step; Programming error:"
  ...
  // -mjd, 21-July-2004
  {
    ...
    if(!cstate.GetDerivedData("dE/dt",current_dE_dt)) {
      throw Oxs_ExtError(this,"Oxs_Spin_SH_STT_Ox_Evolve::Step:"
    ...
    if(new_energy_and_dmdt_computed) {
      ...
      if(!nstate.AddDerivedData("Timestep lower bound",
        ...
        throw Oxs_ExtError(this,
                           "Oxs_Spin_SH_STT_Ox_Evolve::Step:"
                           " Programming error; data cache already set.");
      }
    }
...
void Oxs_Spin_SH_STT_Ox_Evolve::UpdateDerivedOutputs(const Oxs_SimState& state)
{ // This routine fills all the Oxs_Spin_SH_STT_Ox_Evolve Oxs_ScalarOutput's to
...
void Oxs_Spin_SH_STT_Ox_Evolve::UpdateSpinTorqueOutputs(const Oxs_SimState& state)
{
  ...
  OC_REAL8m ICSHmult = EvaluateICSHProfileScript(state.stage_number,
                          state.stage_elapsed_time,
                          state.stage_start_time+state.stage_elapsed_time);
...
// Spin-Hall section---------------------------------------------------------------------------------------
      ...
      ThreeVector mxmxs    = m; mxmxs ^= mxs;
      OC_REAL8m   g2_func  = B2[i];
                  g2_func *= Ms_inverse[i];
                  g2_func *= g[i];
                  g2_func *= ICSHmult;
//---------------------------------------------------------------------------------------------------------
// assembling SH-torque term
      ThreeVector scratchB  = mxs;
                  scratchB *= cell_beta;
                  scratchB -= mxmxs;
                  scratchB *= -1.*g2_func;

      scratchA += scratchB;
      scratchA *= cell_gamma; // gamma[i] stores gamma_LL
  ...
  if(ave_ICSH_output.GetCacheRequestCount()>0) {
    ave_ICSH_output.cache.value=ave_ICSH*ICSHmult;
    ave_ICSH_output.cache.state_id = state.Id();
  }
