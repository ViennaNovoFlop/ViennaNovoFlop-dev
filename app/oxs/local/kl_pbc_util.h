...
//    const OC_REAL4m error_p_dip_OFFdiag;  
    ...
    const OC_UINT4m max_no_sum_elements;  
    ...
    const OC_UINT4m no_of_sum_elements;  
    ...
    // mesh parameters
    mutable OC_UINT4m xdim, ydim, zdim; // natural size
    mutable OC_UINT4m xydim;
    mutable OC_UINT4m totalsize;
    ...
    mutable OC_UINT4m progress_cnt;      
    // the same, but now in units, 0..totalsize
    mutable OC_UINT4m cell_cnt;      
...
// STATISTICS end
    ...
    // conceptually const
    OC_REALWIDE CalculateSDA_PBC_really( \
    ...
    // result is positive
    OC_REAL4m CalculateSDAerrIMG_2(const OC_REALWIDE r2, 
                                const T_tens_el teXX) const;
    // similarly for *dip* method
    OC_REAL4m CalculateSDAerrDIP_2(const OC_REALWIDE r2, 
                                const T_tens_el teXX) const;
...
//	      const OC_REAL4m error_p_dip_OFFdiag,
    ...
    // conceptually const                       
    OC_REALWIDE CalculateSDA(const T_tens_el teXX,
