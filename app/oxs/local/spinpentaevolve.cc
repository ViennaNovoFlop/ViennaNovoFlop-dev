...
OXS_EXT_REGISTER(Oxs_SpinPentaEvolve);

/* End includes */

// Constructor
Oxs_SpinPentaEvolve::Oxs_SpinPentaEvolve(
...
//fprintf(f,"Flag #1\n");
  ...
  // Process arguments
  if(HasInitValue("P1")) {
    OXS_GET_INIT_EXT_OBJECT("P1",Oxs_ScalarField,P1_init);
  } else {
      // Default setting: create a uniform scalar field
      // If not defined thrwo an error message
      // 21-Jan-2013, TW
      throw Oxs_ExtError(this,"Error Oxs_SpinPentaEvolve: No polarization value P1 for first interface defined.\n");
    }
  if(HasInitValue("P2")) {
    OXS_GET_INIT_EXT_OBJECT("P2",Oxs_ScalarField,P2_init);
  } else {
      // Default setting: create a uniform scalar field
      // If not defined thrwo an error message
      // 21-Jan-2013, TW
      throw Oxs_ExtError(this,"Error Oxs_SpinPentaEvolve: No polarization value P2 for second interface defined.\n");
    }

 
  interface1_type=IT_INVALID; 
  if(HasInitValue("Interface1")) {
      String interface1_type_str = GetStringInitValue("Interface1","IT_INVALID");
      if(interface1_type_str.compare("METAL")==0)         {interface1_type=IT_METAL;}
      else if (interface1_type_str.compare("OXIDE")==0)   {interface1_type=IT_OXIDE;}
      else {
      throw Oxs_ExtError(this,"Error Oxs_SpinPentaEvolve: Invalid interface type for first interface defined.\n Only METAL and OXIDE allowed.\n ");
      };
  } else {
      // If not defined at all throw an error message
      // 21-Jan-2013, TW
      throw Oxs_ExtError(this,"Error Oxs_SpinPentaEvolve: No interface type for first interface defined.\n");
    }

  interface2_type=IT_INVALID; 
  if(HasInitValue("Interface2")) {
      String interface2_type_str = GetStringInitValue("Interface2","IT_INVALID");
      if(interface2_type_str.compare("METAL")==0)         {interface2_type=IT_METAL;}
      else if (interface2_type_str.compare("OXIDE")==0)   {interface2_type=IT_OXIDE;}
      else {
      throw Oxs_ExtError(this,"Error Oxs_SpinPentaEvolve: Invalid interface type for second interface defined.\n Only METAL and OXIDE allowed.\n ");
      };
  } else {
      // If not defined at all throw an error message
      // 21-Jan-2013, TW
      throw Oxs_ExtError(this,"Error Oxs_SpinPentaEvolve: No interface type for second interface defined.\n");
    }
  ...
  if(HasInitValue("g")) {
    OXS_GET_INIT_EXT_OBJECT("g",Oxs_ScalarField,g_init);
      ...
      // 10-Jan-2013, TW
      Oxs_Ext* foo=MakeNew("Oxs_UniformScalarField",director,"value 2.00231930436153");
  ...
  if(HasInitValue("mp1")) {
  OXS_GET_INIT_EXT_OBJECT("mp1",Oxs_VectorField,mp1_init);
  } else {
           throw Oxs_ExtError(this,"Error Oxs_SpinPentaEvolve: No electron polarization vector mp1 for first interface defined.\n");
         }
  
  if(HasInitValue("mp2")) {
  OXS_GET_INIT_EXT_OBJECT("mp2",Oxs_VectorField,mp2_init);
  } else {
           throw Oxs_ExtError(this,"Error Oxs_SpinPentaEvolve: No electron polarization vector mp2 for second interface defined.\n");
         }
  ...
  //  alpha1_init.SetAsOwner(dynamic_cast<Oxs_ScalarField *>
  //                        (MakeNew("Oxs_UniformScalarField",director,
  //                                 "value 0.5")));
      throw Oxs_ExtError(this,"Error Oxs_SpinPentaEvolve: No damping value alpha defined.\n");
  }
  ...
  if(method.compare("rk2")==0) {
    rkstep_ptr = &Oxs_SpinPentaEvolve::TakeRungeKuttaStep2;
  } else if(method.compare("rk2heun")==0) {
    rkstep_ptr = &Oxs_SpinPentaEvolve::TakeRungeKuttaStep2Heun;
  } else if(method.compare("rk4")==0) {
    rkstep_ptr = &Oxs_SpinPentaEvolve::TakeRungeKuttaStep4;
  } else if(method.compare("rkf54m")==0) {
    rkstep_ptr = &Oxs_SpinPentaEvolve::TakeRungeKuttaFehlbergStep54M;
  } else if(method.compare("rkf54s")==0) {
    rkstep_ptr = &Oxs_SpinPentaEvolve::TakeRungeKuttaFehlbergStep54S;
  } else if(method.compare("rkf54")==0) {
    rkstep_ptr = &Oxs_SpinPentaEvolve::TakeRungeKuttaFehlbergStep54;
  } else {
    throw Oxs_ExtError(this,"Invalid initialization detected:"
  ...
  max_dm_dt_output.Setup(this,InstanceName(),"Max dm/dt","deg/ns",0,
     &Oxs_SpinPentaEvolve::UpdateDerivedOutputs);
  ...
  ave_J_output.Setup(this,InstanceName(),"average J","A/m^2",0,
     &Oxs_SpinPentaEvolve::UpdateSpinTorqueOutputs);
...
OC_BOOL Oxs_SpinPentaEvolve::Init()
{
  ...
  B.Release();
  A.Release();
  polarization1.Release();
  polarization2.Release();
  mesh_id = 0;
...
void Oxs_SpinPentaEvolve::UpdateMeshArrays(const Oxs_Mesh* mesh)
{
  mesh_id = 0; // Mark update in progress
  const Oxs_RectangularMesh* rmesh
    = dynamic_cast<const Oxs_RectangularMesh*>(mesh);
  if(rmesh==NULL) {
    String msg="Import mesh to Oxs_SpinPentaEvolve::UpdateMeshArrays"
  ...
  P1_init->FillMeshValue(mesh,polarization1);
  P2_init->FillMeshValue(mesh,polarization2);
  ...
  for(j=0;j<fixed_count;j++) {
    ...
    polarization1[it]  = 0.0;
    polarization2[it]  = 0.0;
    B[i] = 0.0;
  }

  mp1_init->FillMeshValue(mesh,mp1);
  mp2_init->FillMeshValue(mesh,mp2);
  for(i=0;i<size;i++) {mp1[i].MakeUnit();mp2[i].MakeUnit();} // These are supposed
  /// to be unit vectors!
...
OC_REAL8m Oxs_SpinPentaEvolve::EvaluateJProfileScript
(OC_UINT4m stage,
...
OC_REAL8m
Oxs_SpinPentaEvolve::PositiveTimestepBound
(OC_REAL8m max_dm_dt)
...
void Oxs_SpinPentaEvolve::Calculate_dm_dt
...
{ // Imports: state, mxH_, pE_pt_
  ...
  for(i=0;i<size;i++) {
    if(Ms_[i]==0) {
      ...
      ThreeVector p1 = mp1[i];
      ThreeVector p2 = mp2[i];
      OC_REAL8m g_1 = 0.;
      OC_REAL8m g_2 = 0.;
      ThreeVector m = spin_[i];
      ThreeVector mxp1 = m; mxp1 ^= p1;
      ThreeVector mxp2 = m; mxp2 ^= p2;
      ThreeVector mxmxp1 = m; mxmxp1 ^= mxp1;
      ThreeVector mxmxp2 = m; mxmxp2 ^= mxp2;
      ThreeVector mxH = dm_dt_[i];
      ThreeVector mxmxH = m; mxmxH ^= mxH;
      OC_REAL8m   coef1 = -1.;
// Adds units and also scales to frequency 1/s
      OC_REAL8m   pre_factor = B[i];
      pre_factor  *= Jmult;
      pre_factor  *= Ms_inverse_[i];
      pre_factor  *= g[i];
     
      if(interface1_type==IT_OXIDE) {
// Slonczewski function g(m.p) for dielectric
      OC_REAL8m mdotp1  = (m*p1);
      mdotp1 *= polarization1[i];
      mdotp1 *= polarization1[i];
      mdotp1 += 1.;
      g_1 = 0.5 * polarization1[i];
      g_1 /= mdotp1;
      }
      if(interface1_type==IT_METAL) {
// Slonczewski function g(m.p) for metal
      OC_REAL8m mdotp1  = (m*p1);
      mdotp1 += 3.;
      mdotp1 *= (1+polarization1[i]);
      mdotp1 *= (1+polarization1[i]);
      mdotp1 *= (1+polarization1[i]);
      mdotp1 /= 4*pow(polarization1[i],1.5);
      mdotp1 -= 4.;
      g_1 = 1.;
      g_1 /= mdotp1;
      }
      if(interface2_type==IT_OXIDE) {
// Slonczewski function g(m.p) for dielectric
      OC_REAL8m mdotp2  = (m*p2);
      mdotp2 *= polarization2[i];
      mdotp2 *= polarization2[i];
      mdotp2 += 1.;
      g_2 = 0.5 * polarization2[i];
      g_2 /= mdotp2;
      }
      if(interface2_type==IT_METAL) {
// Slonczewski function g(m.p) for metal
      OC_REAL8m mdotp2  = (m*p2);
      mdotp2 += 3.;
      mdotp2 *= (1+polarization2[i]); 
      mdotp2 *= (1+polarization2[i]); 
      mdotp2 *= (1+polarization2[i]); 
      mdotp2 /= 4*pow(polarization2[i],1.5);
      mdotp2 -= 4.;
      g_2 = 1.;
      g_2 /= mdotp2;
      }
...
// starting to assemble the complete term -> g_func*[alpha*mxp-mxmxp]
      ThreeVector scratch1 = mxp1;
      scratch1 *= cell_alpha;
      scratch1 -= mxmxp1;
      scratch1 *= g_1;
      ThreeVector scratch2 = mxp2;
      scratch2 *= cell_alpha;
      scratch2 -= mxmxp2;
      scratch2 *= g_2;
      scratch1 -= scratch2;// second interface must exhibit opposite sign due to geometry!!!
...
//      idm_dt_[i] = gamma[i] * ( mxH + cell_alpha * mxmxH);
      ...
      p1 -= p2;
      p1 *= pre_factor;
      p1 += coef1 * cell_alpha * mxH;
// Energy change
      dE_dt_sum += (p1*mxH)*Ms_[i]*mesh->Volume(i)*cell_mgamma;
     // dE_dt_sum += (cell_alpha*(mxH*mxH))*Ms_[i]*mesh->Volume(i)*gamma[i];
      ...
      OC_REAL8m dm_dt_sq = scratch1.MagSq();
...
void Oxs_SpinPentaEvolve::CheckCache(const Oxs_SimState& cstate)
{
  ...
  if(!cache_good) {
    throw Oxs_ExtError(this,
       "Oxs_SpinPentaEvolve::CheckCache: Invalid data cache.");
  }
}


void
Oxs_SpinPentaEvolve::AdjustState
(OC_REAL8m hstep,
 OC_REAL8m mstep,
 ...
 OC_REAL8m& norm_error) const
{
  ...
  // to Oxs_SpinPentaEvolve::Step() takes 2 half-steps, and it is the
  ...
  // in by the Oxs_SpinPentaEvolve::NegotiateTimeStep() method.
}


void Oxs_SpinPentaEvolve::UpdateTimeFields
...
void Oxs_SpinPentaEvolve::NegotiateTimeStep
...
{ // This routine negotiates with driver over the proper step size.
  ...
  if(!cstate.GetDerivedData("Max dm/dt",max_dm_dt)) {
    throw Oxs_ExtError(this,
       "Oxs_SpinPentaEvolve::NegotiateTimeStep: max_dm_dt not cached.");
  }
  ...
  if(!cstate.GetDerivedData("Timestep lower bound",
                            timestep_lower_bound)) {
    throw Oxs_ExtError(this,
       "Oxs_SpinPentaEvolve::NegotiateTimeStep: "
...
OC_BOOL
Oxs_SpinPentaEvolve::CheckError
...
void Oxs_SpinPentaEvolve::TakeRungeKuttaStep2
(OC_REAL8m stepsize,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
    ...
    throw Oxs_ExtError(this,
                         "Oxs_SpinPentaEvolve::TakeRungeKuttaStep2:"
...
void Oxs_SpinPentaEvolve::TakeRungeKuttaStep2Heun
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
     ...
     !endstate->AddDerivedData("dE/dt",dE_dt)) {
    throw Oxs_ExtError(this,
                         "Oxs_SpinPentaEvolve::TakeRungeKuttaStep2Heun:"
                         " Programming error; data cache already set.");
  }
...
void Oxs_SpinPentaEvolve::TakeRungeKuttaStep4
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_SpinPentaEvolve::RungeKuttaFehlbergBase54
(RKF_SubType method,
 OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
...
{ // Runge-Kutta-Fehlberg routine with combined 4th and 5th
  ...
  default:
    throw Oxs_ExtError(this,
                 "Oxs_SpinPentaEvolve::RungeKuttaFehlbergBase54:"
...
void Oxs_SpinPentaEvolve::TakeRungeKuttaFehlbergStep54
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_SpinPentaEvolve::TakeRungeKuttaFehlbergStep54M
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_SpinPentaEvolve::TakeRungeKuttaFehlbergStep54S
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
OC_REAL8m Oxs_SpinPentaEvolve::MaxDiff
...
void Oxs_SpinPentaEvolve::AdjustStepHeadroom(OC_INT4m step_reject)
...
OC_BOOL
Oxs_SpinPentaEvolve::Step(const Oxs_TimeDriver* driver,
                      Oxs_ConstKey<Oxs_SimState> current_state_key,
                      ...
                      Oxs_Key<Oxs_SimState>& next_state_key)
{
  ...
  if(next_timestep<=0.0 ||
    ...
    if(cstate.stage_number==0
      ...
      throw Oxs_ExtError(this,
           "Oxs_SpinPentaEvolve::Step; Programming error:"
  ...
  // -mjd, 21-July-2004
  {
    ...
    if(!cstate.GetDerivedData("dE/dt",current_dE_dt)) {
      throw Oxs_ExtError(this,"Oxs_SpinPentaEvolve::Step:"
...
void Oxs_SpinPentaEvolve::UpdateDerivedOutputs(const Oxs_SimState& state)
{ // This routine fills all the Oxs_SpinPentaEvolve Oxs_ScalarOutput's to
...
void Oxs_SpinPentaEvolve::UpdateSpinTorqueOutputs(const Oxs_SimState& state)
{
...
// multiplying this term with corresponding pre-factor
      scratch1 *= pre_factor;
      scratch1 *= -1*gamma[i]; // gamma[i] stores gamma_LL
      stt[i] = scratch1; // Note that this is zero at fixed spin sites!!!
      }
    }
