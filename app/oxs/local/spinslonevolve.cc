...
OXS_EXT_REGISTER(Oxs_SpinSlonEvolve);

/* End includes */

// Constructor
Oxs_SpinSlonEvolve::Oxs_SpinSlonEvolve(
...
//fprintf(f,"Flag #1\n");
  ...
  if(method.compare("rk2")==0) {
    rkstep_ptr = &Oxs_SpinSlonEvolve::TakeRungeKuttaStep2;
  } else if(method.compare("rk2heun")==0) {
    rkstep_ptr = &Oxs_SpinSlonEvolve::TakeRungeKuttaStep2Heun;
  } else if(method.compare("rk4")==0) {
    rkstep_ptr = &Oxs_SpinSlonEvolve::TakeRungeKuttaStep4;
  } else if(method.compare("rkf54m")==0) {
    rkstep_ptr = &Oxs_SpinSlonEvolve::TakeRungeKuttaFehlbergStep54M;
  } else if(method.compare("rkf54s")==0) {
    rkstep_ptr = &Oxs_SpinSlonEvolve::TakeRungeKuttaFehlbergStep54S;
  } else if(method.compare("rkf54")==0) {
    rkstep_ptr = &Oxs_SpinSlonEvolve::TakeRungeKuttaFehlbergStep54;
  } else {
    throw Oxs_ExtError(this,"Invalid initialization detected:"
  ...
  max_dm_dt_output.Setup(this,InstanceName(),"Max dm/dt","deg/ns",0,
     &Oxs_SpinSlonEvolve::UpdateDerivedOutputs);
  ...
  ave_J_output.Setup(this,InstanceName(),"average J","A/m^2",0,
     &Oxs_SpinSlonEvolve::UpdateSpinTorqueOutputs);
...
OC_BOOL Oxs_SpinSlonEvolve::Init()
{
...
void Oxs_SpinSlonEvolve::UpdateMeshArrays(const Oxs_Mesh* mesh)
{
  mesh_id = 0; // Mark update in progress
  const Oxs_RectangularMesh* rmesh
    = dynamic_cast<const Oxs_RectangularMesh*>(mesh);
  if(rmesh==NULL) {
    String msg="Import mesh to Oxs_SpinSlonEvolve::UpdateMeshArrays"
...
OC_REAL8m Oxs_SpinSlonEvolve::EvaluateJProfileScript
(OC_UINT4m stage,
...
OC_REAL8m
Oxs_SpinSlonEvolve::PositiveTimestepBound
(OC_REAL8m max_dm_dt)
...
void Oxs_SpinSlonEvolve::Calculate_dm_dt
...
void Oxs_SpinSlonEvolve::CheckCache(const Oxs_SimState& cstate)
{
  ...
  if(!cache_good) {
    throw Oxs_ExtError(this,
       "Oxs_SpinSlonEvolve::CheckCache: Invalid data cache.");
  }
}


void
Oxs_SpinSlonEvolve::AdjustState
(OC_REAL8m hstep,
 OC_REAL8m mstep,
 ...
 OC_REAL8m& norm_error) const
{
  ...
  // to Oxs_SpinSlonEvolve::Step() takes 2 half-steps, and it is the
  ...
  // in by the Oxs_SpinSlonEvolve::NegotiateTimeStep() method.
}


void Oxs_SpinSlonEvolve::UpdateTimeFields
...
void Oxs_SpinSlonEvolve::NegotiateTimeStep
...
{ // This routine negotiates with driver over the proper step size.
  ...
  if(!cstate.GetDerivedData("Max dm/dt",max_dm_dt)) {
    throw Oxs_ExtError(this,
       "Oxs_SpinSlonEvolve::NegotiateTimeStep: max_dm_dt not cached.");
  }
  ...
  if(!cstate.GetDerivedData("Timestep lower bound",
                            timestep_lower_bound)) {
    throw Oxs_ExtError(this,
       "Oxs_SpinSlonEvolve::NegotiateTimeStep: "
...
void Oxs_SpinSlonEvolve::TakeRungeKuttaStep2
(OC_REAL8m stepsize,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
    ...
    throw Oxs_ExtError(this,
                         "Oxs_SpinSlonEvolve::TakeRungeKuttaStep2:"
...
void Oxs_SpinSlonEvolve::TakeRungeKuttaStep2Heun
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
     ...
     !endstate->AddDerivedData("dE/dt",dE_dt)) {
    throw Oxs_ExtError(this,
                         "Oxs_SpinSlonEvolve::TakeRungeKuttaStep2Heun:"
                         " Programming error; data cache already set.");
  }
...
void Oxs_SpinSlonEvolve::TakeRungeKuttaStep4
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_SpinSlonEvolve::RungeKuttaFehlbergBase54
(RKF_SubType method,
 OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
...
{ // Runge-Kutta-Fehlberg routine with combined 4th and 5th
  ...
  default:
    throw Oxs_ExtError(this,
                 "Oxs_SpinSlonEvolve::RungeKuttaFehlbergBase54:"
...
void Oxs_SpinSlonEvolve::TakeRungeKuttaFehlbergStep54
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_SpinSlonEvolve::TakeRungeKuttaFehlbergStep54M
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_SpinSlonEvolve::TakeRungeKuttaFehlbergStep54S
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
OC_REAL8m Oxs_SpinSlonEvolve::MaxDiff
...
void Oxs_SpinSlonEvolve::AdjustStepHeadroom(OC_INT4m step_reject)
...
OC_BOOL
Oxs_SpinSlonEvolve::Step(const Oxs_TimeDriver* driver,
                      Oxs_ConstKey<Oxs_SimState> current_state_key,
                      ...
                      Oxs_Key<Oxs_SimState>& next_state_key)
{
  ...
  if(next_timestep<=0.0 ||
    ...
    if(cstate.stage_number==0
      ...
      throw Oxs_ExtError(this,
           "Oxs_SpinSlonEvolve::Step; Programming error:"
  ...
  // -mjd, 21-July-2004
  {
    ...
    if(!cstate.GetDerivedData("dE/dt",current_dE_dt)) {
      throw Oxs_ExtError(this,"Oxs_SpinSlonEvolve::Step:"
...
void Oxs_SpinSlonEvolve::UpdateDerivedOutputs(const Oxs_SimState& state)
{ // This routine fills all the Oxs_SpinSlonEvolve Oxs_ScalarOutput's to
...
void Oxs_SpinSlonEvolve::UpdateSpinTorqueOutputs(const Oxs_SimState& state)
{
