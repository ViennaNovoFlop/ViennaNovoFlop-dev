...
OXS_EXT_REGISTER(Anv_SpinT3DEvolve);

/* End includes */

// Constructor
Anv_SpinT3DEvolve::Anv_SpinT3DEvolve(
...
// getting/checking  J for current direction; twindbach 14.06.2012
  if(HasInitValue("j")) {
   OXS_GET_INIT_EXT_OBJECT("j",Oxs_VectorField,j_init);
    ...
    char buf[4096];
    Oc_Snprintf(buf,sizeof(buf),"SpinT3DEvolve: no j-vector (current direction given) given");
  ...
  if(method.compare("rk2")==0) {
    rkstep_ptr = &Anv_SpinT3DEvolve::TakeRungeKuttaStep2;
  } else if(method.compare("rk4")==0) {
    rkstep_ptr = &Anv_SpinT3DEvolve::TakeRungeKuttaStep4;
  } else if(method.compare("rkf54m")==0) {
    rkstep_ptr = &Anv_SpinT3DEvolve::TakeRungeKuttaFehlbergStep54M;
  } else if(method.compare("rkf54s")==0) {
    rkstep_ptr = &Anv_SpinT3DEvolve::TakeRungeKuttaFehlbergStep54S;
  } else if(method.compare("rkf54")==0) {
    rkstep_ptr = &Anv_SpinT3DEvolve::TakeRungeKuttaFehlbergStep54;
  } else {
    throw Oxs_Ext::Error(this,"Invalid initialization detected:"
  ...
  max_dm_dt_output.Setup(this,InstanceName(),"Max dm/dt","deg/ns",0,
     &Anv_SpinT3DEvolve::UpdateDerivedOutputs);
  ...
  ave_u_output.Setup(this,InstanceName(),"average u","m/s",0,
		 &Anv_SpinT3DEvolve::UpdateSpinTorqueOutputs);
...
OC_BOOL Anv_SpinT3DEvolve::Init()
{
  // Setup outputs
...
void Anv_SpinT3DEvolve::UpdateMeshArrays(const Oxs_Mesh* mesh)
{
  ...
  if(rmesh==NULL) {
    String msg="Import mesh to Anv_SpinT3DEvolve::UpdateMeshArrays"
...
OC_REAL8m Anv_SpinT3DEvolve::EvaluateuProfileScript
(OC_UINT4m stage,
...
void Anv_SpinT3DEvolve::Calculate_dm_dt
...
// try to calculate dm_dy first -> if this works dm_dz
...
// -------------------- here the calculation of u*grady_m ends -----------------------------------------------------
...
void Anv_SpinT3DEvolve::CheckCache(const Oxs_SimState& cstate)
{
  ...
  if(!cache_good) {
    throw Oxs_Ext::Error(this,
       "Anv_SpinT3DEvolve::CheckCache: Invalid data cache.");
  }
}


void
Anv_SpinT3DEvolve::AdjustState
(OC_REAL8m hstep,
 OC_REAL8m mstep,
 ...
 OC_REAL8m& norm_error) const
{
  ...
  // to Anv_SpinT3DEvolve::Step() takes 2 half-steps, and it is the
  ...
  // in by the Anv_SpinT3DEvolve::NegotiateTimeStep() method.
}


void Anv_SpinT3DEvolve::UpdateTimeFields
...
void Anv_SpinT3DEvolve::NegotiateTimeStep
...
{ // This routine negotiates with driver over the proper step size.
  ...
  if(!cstate.GetDerivedData("Max dm/dt",max_dm_dt)) {
    throw Oxs_Ext::Error(this,
       "Anv_SpinT3DEvolve::NegotiateTimeStep: max_dm_dt not cached.");
  }
  ...
  if(!cstate.GetDerivedData("Timestep lower bound",
			    timestep_lower_bound)) {
    throw Oxs_Ext::Error(this,
       "Anv_SpinT3DEvolve::NegotiateTimeStep: "
...
void Anv_SpinT3DEvolve::TakeRungeKuttaStep2
(OC_REAL8m stepsize,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
    ...
    throw Oxs_Ext::Error(this,
			 "Anv_SpinT3DEvolve::TakeRungeKuttaStep2:"
...
void Anv_SpinT3DEvolve::TakeRungeKuttaStep4
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Anv_SpinT3DEvolve::RungeKuttaFehlbergBase54
(RKF_SubType method,
 OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
...
{ // Runge-Kutta-Fehlberg routine with combined 4th and 5th
  ...
  default:
    throw Oxs_Ext::Error(this,
		 "Anv_SpinT3DEvolve::RungeKuttaFehlbergBase54:"
...
void Anv_SpinT3DEvolve::TakeRungeKuttaFehlbergStep54
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Anv_SpinT3DEvolve::TakeRungeKuttaFehlbergStep54M
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Anv_SpinT3DEvolve::TakeRungeKuttaFehlbergStep54S
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
OC_REAL8m Anv_SpinT3DEvolve::MaxDiff
...
void Anv_SpinT3DEvolve::AdjustStepHeadroom(OC_INT4m step_reject)
...
OC_BOOL
Anv_SpinT3DEvolve::Step(const Oxs_TimeDriver* driver,
                      Oxs_ConstKey<Oxs_SimState> current_state_key,
                      ...
                      Oxs_Key<Oxs_SimState>& next_state_key)
{
  ...
  if(next_timestep<=0.0 ||
    ...
    if(cstate.stage_number==0
      ...
      throw Oxs_Ext::Error(this,
	   "Anv_SpinT3DEvolve::Step; Programming error:"
  ...
  if(!cstate.GetDerivedData("dE/dt",current_dE_dt)) {
    throw Oxs_Ext::Error(this,"Anv_SpinT3DEvolve::Step:"
...
void Anv_SpinT3DEvolve::UpdateDerivedOutputs(const Oxs_SimState& state)
{ // This routine fills all the Anv_SpinT3DEvolve Oxs_ScalarOutput's to
...
void Anv_SpinT3DEvolve::UpdateSpinTorqueOutputs(const Oxs_SimState& state)
{
