/* FILE: spinTSRevolve.cc                 -*-Mode: c++-*-
 *
 * Concrete evolver class, is derived from Oxs_SpinXferEvolve which is
 * built on top of the Oxs_RungeKuttaEvolve
...
OXS_EXT_REGISTER(Oxs_SpinTSREvolve);

/* End includes */

// Constructor
Oxs_SpinTSREvolve::Oxs_SpinTSREvolve(
    ...
    rkstep_ptr(NULL)
{
  ...
  if(P_free_init.GetPtr()==NULL) {
    ...
    /// except as part of Oxs_SpinTSREvolve instance
...
/////////////////////////////////////////////////////////////////////////////
// Check if a layer thickness is given and if there is a standard deviation
/////////////////////////////////////////////////////////////////////////////
 
 if(HasInitValue("thickness")) {
    OXS_GET_INIT_EXT_OBJECT("thickness",Oxs_ScalarField,thickness_init);
    } else {
    // Default is zero
    Oxs_Ext* foo=MakeNew("Oxs_UniformScalarField",director,"value 0.0");
    thickness_init.SetAsOwner(dynamic_cast<Oxs_ScalarField*>(foo));
  }

// If the field is filled with zeros it must be initalized with the aid of the 
// standard deviation
   standard_deviation=GetRealInitValue("standard_deviation",0.);
...
////////////////////////////////////////////////////////////////////////////
  ...
  if(method.compare("rk2")==0) {
    rkstep_ptr = &Oxs_SpinTSREvolve::TakeRungeKuttaStep2;
  } else if(method.compare("rk2heun")==0) {
    rkstep_ptr = &Oxs_SpinTSREvolve::TakeRungeKuttaStep2Heun;
  } else if(method.compare("rk4")==0) {
    rkstep_ptr = &Oxs_SpinTSREvolve::TakeRungeKuttaStep4;
  } else if(method.compare("rkf54m")==0) {
    rkstep_ptr = &Oxs_SpinTSREvolve::TakeRungeKuttaFehlbergStep54M;
  } else if(method.compare("rkf54s")==0) {
    rkstep_ptr = &Oxs_SpinTSREvolve::TakeRungeKuttaFehlbergStep54S;
  } else if(method.compare("rkf54")==0) {
    rkstep_ptr = &Oxs_SpinTSREvolve::TakeRungeKuttaFehlbergStep54;
  } else {
    throw Oxs_ExtError(this,"Invalid initialization detected:"
  ...
  max_dm_dt_output.Setup(this,InstanceName(),"Max dm/dt","deg/ns",0,
     &Oxs_SpinTSREvolve::UpdateDerivedOutputs);
  ...
  ave_J_output.Setup(this,InstanceName(),"average J","A/m^2",0,
     &Oxs_SpinTSREvolve::UpdateSpinTorqueOutputs);
...
OC_BOOL Oxs_SpinTSREvolve::Init()
{
...
void Oxs_SpinTSREvolve::UpdateMeshArrays(const Oxs_Mesh* mesh)
{
  ...
  if(rmesh==NULL) {
    String msg="Import mesh to Oxs_SpinTSREvolve::UpdateMeshArrays"
...
/////////////////////////////////////////////////////////////////////
// Layer thickness was uniform in Oxs_SpinXferEvolve
// Here it is only constant for a given column 
////////////////////////////////////////////////////////////////////
...
//  else                  beta_factor = 0.0; // Safety
  ...
  thickness_init->FillMeshValue(mesh,thickness); // write imported thickness scalar field into mesh
  ...
  for(i=0;i<size;i++) {
    ...
    beta /= thickness[i]; // thickness[i] holds the local thickness
    /// This is actually (beta/gamma)*d*Ms

    OC_REAL8m fixed_temp = beta_q_plus[i];
    if(fixed_temp<1.0) {
      char buf[1024];
      Oc_Snprintf(buf,sizeof(buf),"Oxs_SpinTSREvolve::UpdateMeshArrays:"
...
void
Oxs_SpinTSREvolve::Propagate_mp
(const Oxs_Mesh* mesh,
 const Oxs_MeshValue<ThreeVector>& spin)
{
  ...
  if(!mp.CheckMesh(mesh)) {
    throw Oxs_ExtError("mp array not properly initialized"
                         " in Oxs_SpinTSREvolve::Propagate_mp");
  }

  const Oxs_RectangularMesh* rmesh
    = dynamic_cast<const Oxs_RectangularMesh*>(mesh);
  if(rmesh==NULL) {
    String msg="Import mesh to Oxs_SpinTSREvolve::Propagate_mp"
      " is not an Oxs_RectangularMesh object.";
    throw Oxs_ExtError(msg.c_str());
  }
  ...
  if(J_direction == JD_X_NEG || J_direction == JD_X_POS) {
    ...
    for(OC_INDEX k=0;k<kend;++k) {
      for(OC_INDEX j=0;j<jend;++j) {
        OC_INDEX base = rmesh->Index(1,j,k);
        if(!fourpt_derivative) {
          // Use simple 2-pt derivative estimate
          ...
          // Use 4-pt derivate estimate in interior,
          // but fall back to 2-pt estimate on ends.
          // NOTE: Currently only coded up for J_direction = JD_X_*.
          mp[base] = spin[base + 1];
          mp[base] -= spin[base - 1];
          mp[base] *= multiplier;
    ...
    for(OC_INDEX k=0;k<kend;++k) {
      for(OC_INDEX j=1;j<jend-1;++j) {
        OC_INDEX base = rmesh->Index(0,j,k);
...
OC_REAL8m Oxs_SpinTSREvolve::EvaluateJProfileScript
(OC_UINT4m stage,
...
OC_REAL8m
Oxs_SpinTSREvolve::PositiveTimestepBound
(OC_REAL8m max_dm_dt)
...
void Oxs_SpinTSREvolve::Calculate_dm_dt
...
void Oxs_SpinTSREvolve::CheckCache(const Oxs_SimState& cstate)
{
  ...
  if(!cache_good) {
    throw Oxs_ExtError(this,
       "Oxs_SpinTSREvolve::CheckCache: Invalid data cache.");
  }
}


void
Oxs_SpinTSREvolve::AdjustState
(OC_REAL8m hstep,
 OC_REAL8m mstep,
 ...
 OC_REAL8m& norm_error) const
{
  ...
  // to Oxs_SpinTSREvolve::Step() takes 2 half-steps, and it is the
  ...
  // in by the Oxs_SpinTSREvolve::NegotiateTimeStep() method.
}


void Oxs_SpinTSREvolve::UpdateTimeFields
...
void Oxs_SpinTSREvolve::NegotiateTimeStep
...
{ // This routine negotiates with driver over the proper step size.
  ...
  if(!cstate.GetDerivedData("Max dm/dt",max_dm_dt)) {
    throw Oxs_ExtError(this,
       "Oxs_SpinTSREvolve::NegotiateTimeStep: max_dm_dt not cached.");
  }
  ...
  if(!cstate.GetDerivedData("Timestep lower bound",
                            timestep_lower_bound)) {
    throw Oxs_ExtError(this,
       "Oxs_SpinTSREvolve::NegotiateTimeStep: "
...
void Oxs_SpinTSREvolve::TakeRungeKuttaStep2
(OC_REAL8m stepsize,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
    ...
    throw Oxs_ExtError(this,
                         "Oxs_SpinTSREvolve::TakeRungeKuttaStep2:"
...
void Oxs_SpinTSREvolve::TakeRungeKuttaStep2Heun
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
{ // This routine performs a second order Runge-Kutta step, with
  ...
  if(!endstate->AddDerivedData("Timestep lower bound",
     ...
     !endstate->AddDerivedData("dE/dt",dE_dt)) {
    throw Oxs_ExtError(this,
                         "Oxs_SpinTSREvolve::TakeRungeKuttaStep2Heun:"
                         " Programming error; data cache already set.");
  }
...
void Oxs_SpinTSREvolve::TakeRungeKuttaStep4
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_SpinTSREvolve::RungeKuttaFehlbergBase54
(RKF_SubType method,
 OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
...
{ // Runge-Kutta-Fehlberg routine with combined 4th and 5th
  ...
  default:
    throw Oxs_ExtError(this,
                 "Oxs_SpinTSREvolve::RungeKuttaFehlbergBase54:"
...
void Oxs_SpinTSREvolve::TakeRungeKuttaFehlbergStep54
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_SpinTSREvolve::TakeRungeKuttaFehlbergStep54M
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
void Oxs_SpinTSREvolve::TakeRungeKuttaFehlbergStep54S
(OC_REAL8m stepsize,
 Oxs_ConstKey<Oxs_SimState> current_state_key,
 const Oxs_MeshValue<ThreeVector>& current_dm_dt,
 Oxs_Key<Oxs_SimState>& next_state_key,
...
OC_REAL8m Oxs_SpinTSREvolve::MaxDiff
...
void Oxs_SpinTSREvolve::AdjustStepHeadroom(OC_INT4m step_reject)
...
OC_BOOL
Oxs_SpinTSREvolve::Step(const Oxs_TimeDriver* driver,
                      Oxs_ConstKey<Oxs_SimState> current_state_key,
                      ...
                      Oxs_Key<Oxs_SimState>& next_state_key)
{
  ...
  if(next_timestep<=0.0 ||
    ...
    if(cstate.stage_number==0
      ...
      throw Oxs_ExtError(this,
           "Oxs_SpinTSREvolve::Step; Programming error:"
  ...
  // -mjd, 21-July-2004
  {
    ...
    if(!cstate.GetDerivedData("dE/dt",current_dE_dt)) {
      throw Oxs_ExtError(this,"Oxs_SpinTSREvolve::Step:"
...
void Oxs_SpinTSREvolve::UpdateDerivedOutputs(const Oxs_SimState& state)
{ // This routine fills all the Oxs_SpinTSREvolve Oxs_ScalarOutput's to
...
void Oxs_SpinTSREvolve::UpdateSpinTorqueOutputs(const Oxs_SimState& state)
{
