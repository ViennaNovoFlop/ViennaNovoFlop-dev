 ...
 ) const
{
  ...
  if(integration_method == QUAD_INTEG) {
	...
	OC_UINT4m i = mesh->Index(0,y,z); // Get base linear address
	...
	field[i+2]  *= 26./24.;
      ...
      OC_UINT4m i = mesh->Index(0,0,0); // Get base linear address
      for(z=0;z<zdim;z++) for(y=0;y<ydim;y++) {
	energy[i]   *= 26./24.;
	energy[i+1] *= 21./24.;
	energy[i+2] *= 26./24.;
    ...
    if(ydim>=6) {
      for(z=0;z<zdim;z++) {
	// Front face
	OC_UINT4m i = mesh->Index(0,0,z); // Get base linear address
