...
// Helper function
OC_UINT4m Klm_Demag_PBC::NextPowerOfTwo(OC_UINT4m n) const
...
{ // This routine is conceptually const.
  ...
  OC_UINT4m index,i,j,k,istop,jstop,kstop;
  ...
  for(k=0;k<kstop;k++) {
    OC_UINT4m kindex = k*rstridez;
    OC_REALWIDE z = 0.0;
    if(k==0) z = -dz;
    else if(k>1) z = dz*(k-1);
    for(j=0;j<jstop;j++) {
      OC_UINT4m jkindex = kindex + j*rstridey;
      OC_REALWIDE y = 0.0;
      if(j==0) y = -dy;
...
fprintf(stderr,"Nxy(-1,1,0,1,2,3)=%.17g   Nxy(1,-1,0,2,1,3)=%.17g\n",
  ...
  for(k=0;k<rdimz;k++) for(j=0;j<rdimy;j++) for(i=0;i<rdimx;i++) {
    ...
    OC_REALWIDE a00= scale*demag_pbc.CalculateSDA(n00, i, j, k); 
    OC_REALWIDE a01= scale*demag_pbc.CalculateSDA(n01, i, j, k); 
    OC_REALWIDE a02= scale*demag_pbc.CalculateSDA(n02, i, j, k); 
  ...
  for(k=0;k<rdimz;k++) for(j=0;j<rdimy;j++) for(i=0;i<rdimx;i++) {
    ...
    OC_REALWIDE a11= scale*demag_pbc.CalculateSDA(n11, i, j, k); 
    OC_REALWIDE a12= scale*demag_pbc.CalculateSDA(n12, i, j, k); 
    OC_REALWIDE a22= scale*demag_pbc.CalculateSDA(n22, i, j, k); 
