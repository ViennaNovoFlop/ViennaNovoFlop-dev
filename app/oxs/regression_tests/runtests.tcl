...
#   MIF files are stored in the examples and bug directories.
# as identified by the examples_dir and bug_dir variables.
#   Check result files are stored in the load and bug directories,
# respectively, for the examples and bug tests.  These are ODT
# and OVF files.
#   Parameter spec files are stored in the load and bug directories,
...
#   Tests from the examples directory are run through boxsi with
# the "-regression_test 1" option.  This option disables default
# output and stopping criteria.  Instead, an ODT file is generated
# and filled with output from each step, and the stage and total
# iteration limits are set to small values.  The main purpose of
...
#   All tests are checked to see that boxsi exits without error.
# In addition, checks are made against output files.  For "load"
# tests (i.e., those from MIF files from the examples directory),
# an ODT output file is assumed, and comparison is made against it.
# For "bug" tests, the results directory is checked for *.odt and
# *.o?f files, and comparisons are made only against such existing
# results files.
#
########################################################################
# ADDING NEW TESTS
...
set PID_INFO_TIMEOUT 30   ;# Max time to wait for mmArchive to start,
                          ## in seconds.
...
proc Usage {} {
   ...
   puts stderr "  -noexcludes ignores exclude files"
...
# Compare loadtests and bugtests to requested tests in argv, and
# construct a list of tests to do.  Each element of dotests is a list of
# the form
#
...
if {[llength $argv]==0} {
   ...
   foreach test $argv {
      ...
      if {[llength $loadmatchlist] == 0 && [llength $bugmatchlist] == 0} {
         puts stderr "Skipping unrecognized command line test \"$test\""
      }
      set test [lreplace $test 0 0]
      set test [regsub -all {,} $test { }]  ;# Replace commas with spaces
...
proc TestCompareODT { oldfile newfile suberrors } {
   ...
   global oldskiplabels newskiplabels swaplabels
   ...
   set newskipcols {}
   if {[llength $newskiplabels] || [llength $swaplabels]} {
...
proc TestCompareOVF { oldfile newfile } {
   ...
   if {[catch {
         set results [eval exec $exec_ignore_stderr \
                         {$TCLSH $OOMMF avfdiff -info $oldfile $newfile}]
      } errmsg]} {
...
proc ExcludeTests { testlist } {
   ...
   global noexcludes examples_dir load_dir bug_dir skipped_tests
   if {$noexcludes} { return $testlist }
   set excludelist {}
   source [file join $load_dir exclude.tcl]
   set loadexcludes [lsort $excludelist]
   set excludelist {}
   source [file join $bug_dir exclude.tcl]
   set bugexcludes [lsort $excludelist]
   set excludelist [concat $loadexcludes $bugexcludes]
   ...
   foreach excludetest $excludelist {
      ...
      foreach index $matchlist {
         if {[llength [lindex $excludetest 1]]==0} {
            ...
            set lines [split [read -nonewline $chan] "\n"]
            close $chan
...
set boxsi_base_command [linsert $boxsi_base_command 0 exec]
...
foreach test $dotests {
   ...
   foreach subdesc $testlist {
      ...
      puts [format "Running total: $tstfmt passed, $tstfmt failed,\
                    elapsed time: %d:%02d" \
               $passedcount [expr {$runerrcount + $comperrcount}] \
...
puts "Tests failed: [expr {$runerrcount + $comperrcount}]"
if {$newcount>0} {
